...
def render_prediction_card(summary: dict):
    ...
    cards_row2 = [
        ...
        ("🤝","BTTS",
         f"{safe_html(str(summary.get('btts_pred','N/A')))} ({summary.get('confidence_btts',0):.0%})" if summary.get('confidence_btts') is not None else "N/A",
         btts_c(summary.get('confidence_btts',0) if summary.get('confidence_btts') is not None else 0.5)),
        ("💲","Stake",
         f"Rp{stake_val:,.0f}" if stake_val>0 else "Rp0",
         "#16a34a" if stake_val>0 else "#6b7280"),
    ]
    render_horizontal_metric_row(cards_row2)
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    if odds_1x2_dict:
        ...
        # Hitung EV berdasarkan hybrid dan odds pasar (dari file)
    ...
    return {
        ...
        "market_odds_1x2": odds_1x2_dict,   # langsung dari file
