...
def render_prediction_card(summary: dict):
    ...
    cards_row2 = [
        ...
        ("🤝", "BTTS",
         rec_btts if rec_btts else (safe_html(str(summary.get('btts_pred','N/A'))) + f" ({summary.get('confidence_btts',0):.0%})" if summary.get('confidence_btts') is not None else "N/A"),
    ...
    # Stake BTTS di bawah jika ada rekomendasi
    if rec_btts and rec_btts != 'NO BET':
        st.markdown(f"<div style='text-align:center;'><span style='background:#16a34a; color:white; padding:2px 10px; border-radius:8px; font-size:0.9rem;'>💵 Stake BTTS: Rp{stake_btts:,.0f}</span></div>", unsafe_allow_html=True)
    ...
    if summary.get('prob_btts') is not None and not summary.get('recommendation_btts'):
        # Hanya tampilkan fair odds dan value analysis jika tidak ada rekomendasi BTTS
...
def render_btts_history(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        ...
        if pred in ('YES', 'NO') and stake > 0:
            ...
            if market_odds > 0:
                if (pred == 'YES' and actual_btts == 1) or (pred == 'NO' and actual_btts == 0):
                    profit = stake * (market_odds - 1)
                    win_count += 1
                else:
                    profit = -stake
                ...
                df.at[idx, 'profit_btts'] = 0.0
                df.at[idx, 'result_btts'] = 'N/A'
        else:
            df.at[idx, 'profit_btts'] = 0.0
            df.at[idx, 'result_btts'] = 'N/A'

    st.metric("Total Profit BTTS", f"Rp {total_profit:+,.0f}")
    if total_bets > 0:
        win_rate = win_count / total_bets * 100
        roi = total_profit / (total_bets * 100000) * 100
        col1, col2, col3 = st.columns(3)
