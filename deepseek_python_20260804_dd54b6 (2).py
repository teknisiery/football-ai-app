...
def load_css():
    ...
    @media (max-width: 768px) {
        .brain-card .label { font-size: 0.5rem; }
...
def get_cs_recommendations(top3_scores, odds_dict):
    ...
    for h, a, prob in top3_scores:
        ...
        if odds is None:
            odds = odds_dict.get("OTHER")  # fallback
        if odds is not None:
            recs.append((h, a, odds))
    return recs[:3]

# --- Fungsi konversi ke format wide untuk arsip ---
def convert_odds_to_wide(match_uid, file_content):
    """Menghasilkan dictionary satu baris dengan kolom odds lebar."""
    df = pd.read_csv(BytesIO(file_content))
...
def render_prediction_card(summary: dict):
    ...
    if summary.get('prob_btts') is not None:
        ...
        if 0 < prob < 1:
            ...
            # --- Input Market Odds BTTS & Value Analysis ---
    ...
    # Gabungan Top 3 Skor & Rekomendasi Correct Score (menggunakan brain-card)
    if summary.get('top3_scores'):
        top3 = summary['top3_scores']
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({info})"):
            ...
            if st.session_state.get(f"processing_{match_uid}", False):
                ...
                if valid:
                    ...
                    # --- Hitung profit Over/Under ---
                    temp_row = {
                        'home_goals': ft_h_val,
                        'away_goals': ft_a_val,
                        'current_ou': full_record.get('current_ou', 2.5),
                        'current_over_odds': full_record.get('current_over_odds', 1.0),
                        'current_under_odds': full_record.get('current_under_odds', 1.0),
                        'recommendation': full_record.get('recommendation', ''),
                        'stake': full_record.get('stake', 100000)
                    }
                    profit_ou, result_ou = ProfitCalculator.calculate(temp_row, 100000)
                    full_record['profit'] = profit_ou
                    full_record['result'] = result_ou

                    # --- Hitung actual_over & actual_btts ---
                    total_goals = ft_h_val + ft_a_val
                    ou_line = float(full_record.get('current_ou', 2.5))
                    if total_goals > ou_line:
                        full_record['actual_over'] = 1
                    elif total_goals < ou_line:
                        full_record['actual_over'] = 0
                    else:
                        full_record['actual_over'] = np.nan
                    full_record['actual_btts'] = 1 if (ft_h_val > 0 and ft_a_val > 0) else 0
