...
def settle_basic(total_goals: int, line: float, bet_type: str) -> str:
    """Hasil dasar: WIN / PUSH / LOSE, dengan memperhatikan bet_type."""
    if bet_type == "OVER":
...
def calculate_profit(row: dict, stake: float = 100000.0) -> Tuple[float, str]:
    """
    Hitung profit dan status taruhan (FULL WIN, HALF WIN, PUSH, HALF LOSE, LOSE, NO BET).
    Mengabaikan pertandingan yang tidak direkomendasikan (NO BET / stake 0).
    """
...
def compute_detailed_profits(history_df: pd.DataFrame):
    ...
    if 'settlement_time' in df.columns:
        ...
        df_bulan['bulan'] = df_bulan['settlement_time'].dt.strftime('%Y-%m')
...
class GitHubStorageProvider(StorageProvider):
    ...
    def _crud(self, method, r, data=None):
        ...
        if resp.status_code == 409:
            # Ambil SHA terbaru dan coba sekali lagi
            sha = self._get_sha(r)
            if sha:
                payload["sha"] = sha
                resp = requests.request(method, url, headers=self._headers(), json=payload)
                resp.raise_for_status()  # jika masih gagal, lempar error
            else:
...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        ...
        # Bangun string informasi prediksi dan stake
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({info})"):
            ...
            # Flag pemrosesan untuk mencegah klik ganda
            ...
            if st.button("💾 Save Score", key=f"save_{match_uid}", disabled=st.session_state[processing_key]):
                ...
                # Konversi None menjadi 0 untuk penyimpanan
                ...
                if valid:
                    ...
                    # Kembalikan flag agar bisa dikoreksi
                    st.session_state[processing_key] = False
                st.rerun()
    ...
    for _, row in raw.iterrows():
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe}"):
            ...
            with c2:
                ...
                if status == "FULL WIN": color = "#16a34a"
                elif status == "HALF WIN": color = "#65a30d"
                elif status == "PUSH": color = "#6b7280"
                elif status == "HALF LOSE": color = "#f97316"
                elif status == "FULL LOSE": color = "#ef4444"
                elif status == "NO BET": color = "#334155"
                else: color = "#334155"
...
def evaluate_model(storage: StorageProvider) -> dict:
    """Hitung MAE dan RMSE dari history yang sudah lengkap."""
    try:
        df = storage.load_dataframe(ResourceRegistry.HISTORY)
    except Exception:
        return {"mae": None, "rmse": None, "message": "Gagal membaca history."}
    
    if df.empty:
        return {"mae": None, "rmse": None, "message": "History kosong."}
    
    if 'totalgol_ft' not in df.columns:
        if 'home_goals' in df.columns and 'away_goals' in df.columns:
            df['totalgol_ft'] = df['home_goals'] + df['away_goals']
        else:
            return {"mae": None, "rmse": None, "message": "Data tidak memiliki kolom skor (home_goals/away_goals)."}
    
    if 'expected_goal' not in df.columns:
        return {"mae": None, "rmse": None, "message": "Data tidak memiliki kolom expected_goal."}
    
    valid = df.dropna(subset=['expected_goal', 'totalgol_ft'])
    if len(valid) < 2:
        return {"mae": None, "rmse": None, "message": "Belum cukup data untuk evaluasi (minimal 2 data)."}
    
    y_true = valid['totalgol_ft']
    y_pred = valid['expected_goal']
    
    mae = mean_absolute_error(y_true, y_pred)
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    return {"mae": mae, "rmse": rmse, "message": None}
...
def render_learning(session: SessionManager, storage: StorageProvider):
    ...
    if st.button("🚀 Latih Ulang Model"):
        with st.spinner("Melatih..."):
            ...
            st.cache_data.clear()   # bersihkan cache evaluasi
    ...
    if eval_result["message"]:
        ...
        col2.metric("📐 RMSE", f"{eval_result['rmse']:.2f}")
        st.caption(
            "MAE: rata‑rata selisih absolut prediksi gol dengan gol aktual. "
            "RMSE: akar rata‑rata kuadrat selisih (memberi bobot lebih pada kesalahan besar). "
            "Semakin kecil semakin baik."
        )

# ============================================================
# REMAINING PAGES (unchanged except render_learning above)
# ============================================================
