...
# + Perbaikan: Nama liga tidak tertimpa + Cek duplikasi History di Save Pending
# ============================================================
...
def calculate_profit(row: dict, stake: float = 100000.0) -> Tuple[float, str]:
    ...
    Menangani NaN dengan fallback aman.
    """
...
def compute_detailed_profits(history_df: pd.DataFrame):
    ...
    profit_by_status = df.groupby('result')['profit'].sum().to_dict()
    ...
    if 'settlement_time' in df.columns:
        ...
        df['bulan'] = df['bulan'].fillna('Tanpa Tanggal')
    ...
    return total_profit, df, summary, monthly_data, profit_by_status
...
def update_league_profile(storage: StorageProvider, league_code: int):
    ...
    # Jaga nama liga agar tidak ditimpa
    existing_name = None
...
def run_analysis(session: SessionManager, storage: StorageProvider, predictor: Predictor):
    ...
    # Untuk dataset_ou.csv: hapus league_name
    df_clean = df.copy()
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({row['prediction_status']})"):
            ...
            if st.button("💾 Save Score", key=f"save{idx}", disabled=st.session_state[save_key]):
                ...
                if valid:
                    ...
                    # Siapkan DataFrame untuk history (dengan league_name)
                    hist_record_df = pd.DataFrame([full_record])
                    hist_record_df = reorder_columns(hist_record_df)
                    
                    # Siapkan DataFrame untuk dataset (tanpa league_name)
