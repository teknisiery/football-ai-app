...
def main():
    ...
    with st.sidebar:
        ...
        # ---- Edit Statistik Liga (menggunakan league_code) ----
        with st.expander("✏️ Edit Statistik Liga"):
            ...
            if profil_league.empty:
                ...
                selected_league_str = st.selectbox("Pilih Liga", league_options, key="edit_league")
                selected_code = int(selected_league_str.split(']')[0].replace('[', ''))
                liga_row = profil_league[profil_league['league_code'] == selected_code].iloc[0]
                ...
                if st.button("💾 Simpan Statistik"):
                    profil_league.loc[profil_league['league_code'] == selected_code, 'league_avg_goals'] = new_avg
                    profil_league.loc[profil_league['league_code'] == selected_code, 'league_over25_pct'] = new_over25
                    profil_league.loc[profil_league['league_code'] == selected_code, 'league_btts_pct'] = new_btts_pct
                    profil_league.loc[profil_league['league_code'] == selected_code, 'league_under35_pct'] = new_under35
                    profil_league.loc[profil_league['league_code'] == selected_code, 'home_win_pct'] = new_home_win
                    profil_league.loc[profil_league['league_code'] == selected_code, 'away_win_pct'] = new_away_win
                    profil_league.loc[profil_league['league_code'] == selected_code, 'draw_pct'] = new_draw
                    db_storage.save_dataframe(ResourceRegistry.LEAGUE_PROFILE, profil_league)
            ...
            if combined_file:
                try:
                    ...
                    if combined.get('cs'):
                        ...
                        # Ekstrak hanya bagian Correct Score dari file gabungan
