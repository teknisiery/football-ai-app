...
# + Perbaikan: Reset state saat ganti file + Analisis otomatis
# ============================================================
...
def update_league_profile(storage: StorageProvider, league_code: int):
    ...
    # Pertahankan nama liga yang sudah ada
    existing_name = None
...
def render_upload_section(session: SessionManager):
    with st.expander("📤 Upload File CSV", expanded=True):
        ...
        if f:
            ...
            # === RESET STATE PREDIKSI & TANDAI ANALISIS OTOMATIS ===
...
def main():
    ...
    if 'uploaded_file' not in st.session_state:
        ...
        if st.button("🔄 Ganti File"):
            ...
            session.set_prediction_state(PredictionState())   # reset state
        ...
        if predictor is None:
            ...
            # === ANALISIS OTOMATIS SETELAH UPLOAD ===
            ...
            # Tombol manual sebagai fallback jika otomatis gagal
