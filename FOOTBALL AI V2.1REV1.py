# ============================================================
# FOOTBALL AI V2 – PRODUCTION (DUAL STORAGE) – REFACTORED
# Task 1-3 + perbaikan awal Task 4 & 6
# ============================================================
...
# ============================================================
# THRESHOLD SERVICE (Task 1)
# ============================================================
class ThresholdService:
    @staticmethod
    def get_thresholds(storage: Optional[Any] = None) -> Tuple[float, float]:
        """
        Ambil threshold EV dari storage pusat, fallback ke file lokal atau default.
        """
...
# ============================================================
# PROFIT CALCULATOR (Task 2) – satu sumber kebenaran
# ============================================================
class ProfitCalculator:
    @staticmethod
    def calculate(row: dict, stake: float = 100000.0) -> Tuple[float, str]:
        """
        Hitung profit dan status taruhan.
        """
...
# ============================================================
# COMPUTE PROFITS (Refactored menggunakan ProfitCalculator)
# ============================================================
...
def get_league_profile(storage: StorageProvider, session: 'SessionManager') -> pd.DataFrame:
    """Mengambil profil liga, dengan cache di AppState."""
    state = session.state
...
# ============================================================
# LIVE BET RECOMMENDATION (dengan ThresholdService)
# ============================================================
def calculate_live_recommendation(
    ...
    # ... (implementasi sama, hanya threshold dari ThresholdService)
...
# ============================================================
# SESSION MANAGER (Task 3: AppState terpusat)
# ============================================================
@dataclass
class UploadState:
    uploaded: bool = False
...
class AppState:
    ...
    uploaded_file: Optional[object] = None      # file uploader object
...
class SessionManager:
    ...
    def set_upload_state(self, s: UploadState):
        self.state.upload = s

    # Prediction
    def get_prediction_state(self) -> PredictionState:
        return self.state.prediction

    def set_prediction_state(self, s: PredictionState):
        self.state.prediction = s

    # Live results
    def get_live_result(self, match_uid: str) -> Optional[dict]:
        return self.state.live_results.get(match_uid)

    def set_live_result(self, match_uid: str, result: dict):
        self.state.live_results[match_uid] = result

    # Analysis flag
    @property
    def analysis_needed(self) -> bool:
        return self.state.analysis_needed

    @analysis_needed.setter
    def analysis_needed(self, val: bool):
        self.state.analysis_needed = val

    # File upload (moved from raw st.session_state)
    @property
    ...
    def uploaded_file(self, file):
        self.state.uploaded_file = file

    @property
    def uploaded_df(self) -> Optional[pd.DataFrame]:
        return self.state.uploaded_df

    @uploaded_df.setter
    def uploaded_df(self, df):
        self.state.uploaded_df = df
    ...
    def get_debug_trace(self) -> List[str]:
        return self.state.debug_trace

    # League profile cache
    def invalidate_league_profile_cache(self):
        self.state.league_profile_cache = None

    # Convenience method to clear analysis after file change
...
def serialize_prediction(result: PredictionResult) -> dict:
    """Ambil field yang relevan untuk penyimpanan."""
    return {
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({info})"):
            ...
            if st.button("💾 Save Score", key=f"save_{match_uid}", disabled=st.session_state[processing_key]):
                if st.session_state[processing_key]:
                    st.warning("Penyimpanan sedang diproses.")
                    st.stop()
                st.session_state[processing_key] = True
                ht_h_val = int(ht_h or 0)
                ht_a_val = int(ht_a or 0)
                ft_h_val = int(ft_h or 0)
                ft_a_val = int(ft_a or 0)
            ...
            live_result = session.get_live_result(match_uid)
...
def evaluate_model(storage: StorageProvider) -> dict:
    # (tidak berubah secara logika, hanya gunakan ProfitCalculator)
    ...
    result = {
        ...
        "cm": cm, "brier": brier, "logloss": logloss,
        ...
        if league_data:
            ...
            for league in sorted(league_data.keys()):
                ...
                with st.expander(f"{league} - Rp {data['profit']:+,.0f}"):
                    ...
                    df_liga = data['df'].sort_values(time_col_liga).dropna(subset=[time_col_liga])
        ...
        if session.get_prediction_state().processed:
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ...
                    # Gunakan serialize_prediction untuk data terstruktur
