# ============================================================
# FOOTBALL AI V2 STABLE – FINAL PRODUCTION
# ============================================================
...
# ============================================================
# PREDICTION CARD & UI (FULL METRICS)
# ============================================================
...
def render_prediction_card(summary):
    ...
    # Baris 1: OU Prediction, Grade, Recommendation
    col_ou, col_grade, col_rec = st.columns(3)
    with col_ou:
        ou_text = f"{summary.get('ou_pred', '')} {summary.get('ou_line', '')}"
        st.markdown(f"<div style='font-size:1.8rem; font-weight:900; margin-bottom:6px;'>{ou_text}</div>", unsafe_allow_html=True)
    with col_grade:
        st.markdown(f'<span class="badge badge-{summary.get("rec_color", "d")}" style="font-size:1rem;">Grade {summary.get("grade", "D")}</span>', unsafe_allow_html=True)
    with col_rec:
        st.markdown(f'<span class="badge badge-{summary.get("rec_color", "d")}">{summary.get("recommendation", "NO BET")}</span>', unsafe_allow_html=True)

    # Baris 2: Stake, Confidence, Edge, Expected Goal
    render_horizontal_metric_row([
        ("💰", "Stake", summary.get('stake', 'Rp0'), "#334155"),
        ("📈", "Confidence", f"{summary.get('confidence_ou', 0):.0%}", "#16a34a"),
        ("📏", "Edge", f"{summary.get('edge', 0):.4f}", "#7c3aed"),
        ("⚽", "Expected Goal", f"{summary.get('expected_goal', 0):.2f}", "#2563eb"),
    ])

    # Baris 3: EV Over/Under, Kelly Over/Under
    ...
    # Baris 4: Football, League, Market, AI
    render_horizontal_metric_row([
        ("⚽", "Football Brain", summary.get('football_brain', 0), "#2563eb"),
        ("🏆", "League Brain", summary.get('league_brain', 0), "#f97316"),
        ("📈", "Market Brain", summary.get('market_brain', 0), "#7c3aed"),
        ("🧠", "AI Score", summary.get('ai_score', 0), "#16a34a"),
    ])

    # Baris 5: BTTS, HT Over 0.5, Historical ROI, Win Rate
    render_horizontal_metric_row([
        ("🤝", "BTTS", f"{summary.get('btts_pred', 'N/A')} ({summary.get('confidence_btts', 0):.0%})" if not np.isnan(summary.get('confidence_btts', np.nan)) else "N/A", "#334155"),
        ("⏱", "HT Over 0.5", f"{summary.get('ht0_pred', 'N/A')} ({summary.get('confidence_ht0', 0):.0%})" if not np.isnan(summary.get('confidence_ht0', np.nan)) else "N/A", "#334155"),
        ("💰", "Historical ROI", f"{summary.get('historical_roi', 0):.1f}%", "#16a34a"),
        ("📊", "Historical Win Rate", f"{summary.get('historical_win_rate', 0):.1f}%", "#16a34a"),
    ])
...
def get_match_prediction_summary():
    ...
    engine = LeagueIndicatorEngine(db.load_league_profile())
