...
def render_prediction_card(summary: dict):
    ...
    # --- 1X2: Tampilan Probabilitas 1X2 (3 card horizontal) ---
    if summary.get('prob_1x2_model'):
        ...
        with col1:
            st.markdown(f"""
            <div class="brain-card" style="background:#1e3a5f; padding:10px; border-radius:10px; color:white;">
                <div class="label" style="font-weight:bold;">Model</div>
                <p>Home: {model.get('home',0):.1%}</p>
                <p>Draw: {model.get('draw',0):.1%}</p>
                <p>Away: {model.get('away',0):.1%}</p>
            </div>
            """, unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
            <div class="brain-card" style="background:#4a4a4a; padding:10px; border-radius:10px; color:white;">
                <div class="label" style="font-weight:bold;">Liga</div>
                <p>Home: {league.get('home',0):.1%}</p>
                <p>Draw: {league.get('draw',0):.1%}</p>
                <p>Away: {league.get('away',0):.1%}</p>
            </div>
            """, unsafe_allow_html=True)
        with col3:
            st.markdown(f"""
            <div class="brain-card" style="background:#2d6a4f; padding:10px; border-radius:10px; color:white;">
                <div class="label" style="font-weight:bold;">Hybrid</div>
                <p>Home: {hybrid_final.get('home',0):.1%}</p>
                <p>Draw: {hybrid_final.get('draw',0):.1%}</p>
                <p>Away: {hybrid_final.get('away',0):.1%}</p>
            </div>
            """, unsafe_allow_html=True)

        fair_odds_1x2 = summary.get('fair_odds_1x2')
        if fair_odds_1x2:
            st.markdown("**Fair Odds 1X2**")
            st.write(f"Home: {fair_odds_1x2.get('home'):.2f}" if fair_odds_1x2.get('home') else "")
            st.write(f"Draw: {fair_odds_1x2.get('draw'):.2f}" if fair_odds_1x2.get('draw') else "")
            st.write(f"Away: {fair_odds_1x2.get('away'):.2f}" if fair_odds_1x2.get('away') else "")
        ...
        # Simpan market odds ke session untuk penyimpanan nanti
        ...
        if hybrid_final:
            ...
            for key, label in [('home', 'Home'), ('draw', 'Draw'), ('away', 'Away')]:
                if fair_odds.get(key):
                    value = market_home if key=='home' else (market_draw if key=='draw' else market_away)
                    diff = value - fair_odds[key]
                    if diff > 0.05:
                        st.success(f"🟢 {label}: Market {value:.2f} > Fair {fair_odds[key]:.2f} (Value +{diff:.2f})")
                    elif diff < -0.05:
                        st.error(f"🔴 {label}: Market {value:.2f} < Fair {fair_odds[key]:.2f} (No Value)")
                    else:
                        st.info(f"⚪ {label}: Market {value:.2f} ≈ Fair {fair_odds[key]:.2f} (Netral)")
...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    # Hanya ambil baris yang memiliki prob 1x2
    req_col = 'prob_1x2_hybrid_home'
    if req_col not in raw.columns:
        st.info("Data 1X2 belum tersedia di history.")
        return

    df = raw.dropna(subset=[req_col]).copy()
    ...
    for _, row in df.iterrows():
        ...
        stake = 100000
        profit = 0.0
        # Tentukan jika ada value bet (market odds > fair odds)
...
def main():
    ...
    with st.sidebar:
        ...
        # ---- Edit Statistik Liga (dengan cache) ----
        with st.expander("✏️ Edit Statistik Liga"):
            try:
                profil_league = db_storage.load_dataframe(ResourceRegistry.LEAGUE_PROFILE)
                if not profil_league.empty:
                    st.session_state['cached_profil'] = profil_league.copy()
            except:
                pass

            profil_league = st.session_state.get('cached_profil')
            if profil_league is None or profil_league.empty:
                st.info("File profil liga tidak tersedia.")
            else:
                if 'cached_profil' not in st.session_state:
                    st.warning("⚠️ Menggunakan data profil liga dari cache lokal.")
        ...
        if session.get_prediction_state().processed:
            ...
            if odds_file:
                try:
                    ...
                    row_data = convert_odds_to_wide(match_uid, odds_file.getvalue())
            ...
            odds_1x2_file = st.file_uploader("📊 Upload Odds 1X2 (CSV)", type=["csv"], key="1x2_upload")
            if odds_1x2_file:
                try:
                    odds_1x2_dict = parse_odds_1x2_csv(odds_1x2_file.getvalue())
                    session.uploaded_odds_1x2 = odds_1x2_dict
                    st.success("Odds 1X2 berhasil diunggah.")
                except Exception as e:
                    st.error(f"Gagal membaca file odds 1X2: {e}")
