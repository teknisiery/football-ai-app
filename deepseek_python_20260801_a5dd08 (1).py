...
class ThresholdService:
    ...
    def get_btts_threshold(storage, league_code):
        """Ambil threshold BTTS untuk liga tertentu dari profil."""
...
def compute_detailed_profits(history_df: pd.DataFrame):
    ...
    if not bet_df.empty:
        # Gunakan _valid_time untuk grouping
    ...
    # Kembalikan profit_df dengan kolom _valid_time agar bisa digunakan di sidebar
    # Simpan ke bet_df, dan bet_df sudah memiliki _valid_time
...
def compute_profits_by_league(history_df: pd.DataFrame):
    ...
    for league, group in league_groups:
        ...
        # Gunakan _valid_time untuk grafik
        league_data[league] = {
            'profit': profit,
            'summary': summary,
            'df': group,
...
class Predictor:
    ...
    def predict(self, features_df: pd.DataFrame) -> PredictionResult:
        ...
        prob_btts = None  # inisialisasi untuk Task 10
...
def main():
    ...
    with st.sidebar:
        ...
        # Gunakan kolom _valid_time jika ada (hasil Task 9)
        ...
        if league_data:
            ...
            for league in sorted(league_data.keys()):
                ...
                with st.expander(f"{league} - Rp {data['profit']:+,.0f}"):
                    ...
                    st.markdown("**Profit per Status**")
                    for status in ['FULL WIN', 'HALF WIN', 'PUSH', 'HALF LOSE', 'FULL LOSE']:
                        st.markdown(f"{status}: Rp {profit_by_status_league.get(status, 0):+,.0f}")
