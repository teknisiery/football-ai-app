...
class AppState:
    ...
    market_odds_1x2: Optional[dict] = None
...
class SessionManager:
    ...
    @property
    def market_odds_1x2(self) -> Optional[dict]:
        return self.state.market_odds_1x2
    @market_odds_1x2.setter
    def market_odds_1x2(self, odds: Optional[dict]):
        self.state.market_odds_1x2 = odds
...
def render_prediction_card(summary: dict):
    ...
    # Tampilkan Fair Odds BTTS jika prob_btts tersedia
    ...
    # --- 1X2: 4 Card Horizontal ---
    if summary.get('prob_1x2_model'):
        model = summary['prob_1x2_model']
        league = summary.get('prob_1x2_league', {})
        hybrid_final = summary.get('prob_1x2_hybrid_final', summary.get('prob_1x2_hybrid', {}))
        fair_odds = summary.get('fair_odds_1x2', {})

        def _1x2_str(d):
            return f"H: {d.get('home',0):.1%}  D: {d.get('draw',0):.1%}  A: {d.get('away',0):.1%}"

        cards = [
            ("🤖", "Model", _1x2_str(model), "#1e3a5f"),
            ("🏟️", "Liga", _1x2_str(league), "#4a4a4a"),
            ("⚡", "Hybrid", _1x2_str(hybrid_final), "#2d6a4f"),
            ("💰", "Fair Odds",
             f"{_1x2_str(hybrid_final)}\nHome: {fair_odds.get('home',0):.2f}  Draw: {fair_odds.get('draw',0):.2f}  Away: {fair_odds.get('away',0):.2f}" if fair_odds else _1x2_str(hybrid_final),
             "#5c4a1f"),
        ]
        render_horizontal_metric_row(cards)

        # Badge Rekomendasi 1X2 (EV+)
        if summary.get('prediction_1x2'):
            rec_1x2 = summary['prediction_1x2']
            rec1x2_color = 'a' if rec_1x2 != 'NO BET' else 'd'
            st.markdown(f"<span class='badge badge-{rec1x2_color}'>1X2: {rec_1x2}</span>", unsafe_allow_html=True)
            if rec_1x2 != 'NO BET' and summary.get('stake_1x2', 0) > 0:
                st.caption(f"Stake 1X2: Rp{summary['stake_1x2']:,}")

        # Input Market Odds 1X2
        st.markdown("**Market Odds 1X2**")
        col_1x2_h, col_1x2_d, col_1x2_a = st.columns(3)
        with col_1x2_h:
            market_home = st.number_input("Home", min_value=1.01, value=2.50, step=0.01, key="market_1x2_home")
        with col_1x2_d:
            market_draw = st.number_input("Draw", min_value=1.01, value=3.20, step=0.01, key="market_1x2_draw")
        with col_1x2_a:
            market_away = st.number_input("Away", min_value=1.01, value=2.80, step=0.01, key="market_1x2_away")

        # Simpan market odds ke session
        st.session_state['_market_odds_1x2_'] = {
            'home': market_home,
            'draw': market_draw,
            'away': market_away
        }
        ...
        if hybrid_final:
            fair_odds_calc = {
                'home': 1/hybrid_final['home'] if hybrid_final['home']>0 else None,
                'draw': 1/hybrid_final['draw'] if hybrid_final['draw']>0 else None,
                'away': 1/hybrid_final['away'] if hybrid_final['away']>0 else None
            }
            st.markdown("**Value Analysis 1X2**")
            for key, label in [('home', 'Home'), ('draw', 'Draw'), ('away', 'Away')]:
                if fair_odds_calc.get(key):
                    value = market_home if key=='home' else (market_draw if key=='draw' else market_away)
                    diff = value - fair_odds_calc[key]
                    if diff > 0.05:
                        st.success(f"🟢 {label}: Market {value:.2f} > Fair {fair_odds_calc[key]:.2f} (Value +{diff:.2f})")
                    elif diff < -0.05:
                        st.error(f"🔴 {label}: Market {value:.2f} < Fair {fair_odds_calc[key]:.2f} (No Value)")
                    else:
                        st.info(f"⚪ {label}: Market {value:.2f} ≈ Fair {fair_odds_calc[key]:.2f} (Netral)")

    # Gabungan Top 3 Skor & Rekomendasi Correct Score (Hybrid)
        ...
        for i, (h, a, prob) in enumerate(top3):
            ...
            if cs_recs and i < len(cs_recs):
                ...
                if rec[0] == h and rec[1] == a:
                    odds = rec[2]
                    stake = [30000, 40000, 50000][i] if i < 3 else 0
                    odds_str = f"Odds: {odds:.2f} | Stake: Rp{stake:,}"
            full_label = f"{label}  {odds_str}" if odds_str else label
            color = "#1e293b"
            cards.append(("⚽", full_label, value, color))
        render_horizontal_metric_row(cards)
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    fair_odds_1x2 = None
    market_odds_1x2 = st.session_state.get('_market_odds_1x2_', None)
    ...
    if odds_1x2_dict:
        fair_1x2 = calculate_fair_probs(odds_1x2_dict)
        if fair_1x2:
        ...
        # Hitung EV berdasarkan hybrid (Model × Liga) dan odds 1X2
        if prob_1x2_hybrid and odds_1x2_dict:
            for outcome in ['home', 'draw', 'away']:
                prob = prob_1x2_hybrid.get(outcome, 0)
    ...
    # Simpan ke r (prediction_result) untuk serialize
    ...
    return {
        ...
        "market_odds_1x2": market_odds_1x2,
...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    # Cek apakah ada data 1X2 baru (dengan prediction_1x2)
    has_new_1x2 = 'prediction_1x2' in raw.columns
    if not has_new_1x2 and 'prob_1x2_hybrid_home' not in raw.columns:
        st.info("Data 1X2 belum tersedia di history.")
        return

    df = raw.copy()
    if 'prob_1x2_hybrid_home' in df.columns:
        df = df.dropna(subset=['prob_1x2_hybrid_home'])
    df['settlement_time'] = pd.to_datetime(df.get('settlement_time'), errors='coerce')
    df = df.sort_values('settlement_time', ascending=False)

    total_profit_1x2 = 0.0
    correct = 0
    total_bets = 0
    ...
    for _, row in df.iterrows():
        ...
        # Jika ada prediction_1x2 dan bukan NO BET, gunakan itu
        pred_1x2 = row.get('prediction_1x2')
        if pred_1x2 and pred_1x2 != 'NO BET' and pd.notna(row.get('stake_1x2')) and float(row.get('stake_1x2', 0)) > 0:
            stake_1x2 = float(row['stake_1x2'])
            # Ambil market odds sesuai prediksi
            market_odds_1x2 = {
                'HOME': float(row.get('market_odds_1x2_home', 0) or 0),
                'DRAW': float(row.get('market_odds_1x2_draw', 0) or 0),
                'AWAY': float(row.get('market_odds_1x2_away', 0) or 0)
            }
            odds = market_odds_1x2.get(pred_1x2, 0)
            if odds > 0:
                if pred_1x2.upper() == actual.upper():
                    profit_1x2 = stake_1x2 * (odds - 1)
                    correct += 1
                else:
                    profit_1x2 = -stake_1x2
                total_bets += 1
                total_profit_1x2 += profit_1x2
            # Jika odds tidak valid, lewati
        else:
            # Fallback ke logika value bet lama jika data 1x2 lama
            if 'prob_1x2_hybrid_home' in row.index:
                prob_home = float(row.get('prob_1x2_hybrid_home', 0))
                prob_draw = float(row.get('prob_1x2_hybrid_draw', 0))
                prob_away = float(row.get('prob_1x2_hybrid_away', 0))
                market_home = float(row.get('market_odds_1x2_home', 0) or 0)
                market_draw = float(row.get('market_odds_1x2_draw', 0) or 0)
                market_away = float(row.get('market_odds_1x2_away', 0) or 0)
                ...
                value_bets = []
                if market_home > fair_home:
                    value_bets.append(('home', market_home))
                if market_draw > fair_draw:
                    value_bets.append(('draw', market_draw))
                if market_away > fair_away:
                    value_bets.append(('away', market_away))

                for bet, odds in value_bets:
                    if bet == actual:
                        total_profit_1x2 += stake * (odds - 1)
                        correct += 1
                    else:
                        total_profit_1x2 -= stake
                    total_bets += 1

    st.metric("Total Profit 1X2", f"Rp {total_profit_1x2:+,.0f}")
    st.metric("Akurasi 1X2", f"{correct}/{total_bets} ({correct/total_bets*100:.1f}%)" if total_bets > 0 else "N/A")
    ...
    for _, row in df.iterrows():
        ...
        prob_home = float(row.get('prob_1x2_hybrid_home', 0)) if 'prob_1x2_hybrid_home' in row.index else 0
        prob_draw = float(row.get('prob_1x2_hybrid_draw', 0)) if 'prob_1x2_hybrid_draw' in row.index else 0
        prob_away = float(row.get('prob_1x2_hybrid_away', 0)) if 'prob_1x2_hybrid_away' in row.index else 0
        market_home = float(row.get('market_odds_1x2_home', 0) or 0)
        market_draw = float(row.get('market_odds_1x2_draw', 0) or 0)
        market_away = float(row.get('market_odds_1x2_away', 0) or 0)
        pred_1x2 = row.get('prediction_1x2')
        st.markdown(f"{home} vs {away}  |  Skor: {score}")
        if pred_1x2 and pred_1x2 != 'NO BET':
            st.caption(f"Prediksi 1X2: {pred_1x2}")
        st.caption(f"Hybrid: {prob_home:.1%} / {prob_draw:.1%} / {prob_away:.1%}  |  Market: {market_home:.2f} / {market_draw:.2f} / {market_away:.2f}")
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("✏️ Edit Statistik Liga"):
            ...
            if profil_league.empty:
                ...
                edit_league = st.selectbox("Pilih Liga", profil_league['league_name'].tolist(), key="edit_league")
                liga_row = profil_league[profil_league['league_name'] == edit_league].iloc[0]
                ...
                if st.button("💾 Simpan Statistik"):
                    profil_league.loc[profil_league['league_name'] == edit_league, 'league_avg_goals'] = new_avg
                    profil_league.loc[profil_league['league_name'] == edit_league, 'league_over25_pct'] = new_over25
                    profil_league.loc[profil_league['league_name'] == edit_league, 'league_btts_pct'] = new_btts_pct
                    profil_league.loc[profil_league['league_name'] == edit_league, 'league_under35_pct'] = new_under35
                    profil_league.loc[profil_league['league_name'] == edit_league, 'home_win_pct'] = new_home_win
                    profil_league.loc[profil_league['league_name'] == edit_league, 'away_win_pct'] = new_away_win
                    profil_league.loc[profil_league['league_name'] == edit_league, 'draw_pct'] = new_draw
                    db_storage.save_dataframe(ResourceRegistry.LEAGUE_PROFILE, profil_league)
    ...
    if session.uploaded_file is None:
            ...
            if combined_file:
                try:
                    ...
                    if combined.get('cs'):
                        ...
                        row_data = convert_odds_to_wide(match_uid, combined_file.getvalue())
                        ...
                        if match_uid not in cs_hist['match_uid'].values if 'match_uid' in cs_hist.columns else True:
        ...
        tab1, tab2, tab2_5, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
            "📋 Pending",
            "📝 Settlement",
            "🎯 1X2",
            "🎯 Correct Score",
