...
from services.resource_registry import Resource, ResourceRegistry       # <-- import baru
...
def evaluate_model(storage) -> dict:
    ...
    if len(class_df) < 2:
        ...
        if 'prob_over' in class_df.columns:
            ...
            logloss = log_loss(y_true_cls, class_df['prob_over'], labels=[0, 1])
        else:
            brier = logloss = None
    ...
    if 'prob_btts' in df.columns and 'home_goals' in df.columns:
        ...
        if len(btts_valid) > 1:
            ...
            logloss_btts = log_loss(btts_valid['btts_actual'], btts_valid['prob_btts'], labels=[0, 1])
