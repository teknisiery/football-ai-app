...
BTTS_FILTER_VERSION = "BTTS_LOW_ODDS_V3_135_OPPOSITE_EV30_NEUTRAL_NO_BET"
BTTS_LOW_ODDS = 1.35
BTTS_OPPOSITE_EV_MIN = 0.30
BTTS_NORMAL_EV_MIN = 0.05
...
def evaluate_btts_filter(
    ...
    +0.30. Otherwise the highest side must clear the normal +0.05 EV floor;
    neutral/no-value markets become NO BET.
    """
    prob = _num(prob_btts)
    ...
    positive = []
    if yes_ev is not None and yes_ev > BTTS_NORMAL_EV_MIN:
        positive.append(("YES", yes_ev))
    if no_ev is not None and no_ev > BTTS_NORMAL_EV_MIN:
    ...
    if positive:
        ...
        result["filtered"] = True
        result["reason"] = "NEUTRAL_OR_EV_TOO_LOW"
