...
def render_learning(session: SessionManager, storage: StorageProvider):
    ...
    if st.button("🚀 Latih Ulang Model"):
        with st.spinner("Melatih..."):
            ...
            model.fit(X, y)
            # Simpan ke app_storage (lokal) agar model langsung dipakai
...
def main():
    ...
    # --- Inisialisasi dua storage ---
    app_storage = LocalStorageProvider()   # untuk model & threshold lokal
    ...
    if st.session_state.get("debug_trace"):
        with st.expander("📜 Raw Debug Trace"):
            for line in st.session_state.debug_trace:
                st.text(line)
