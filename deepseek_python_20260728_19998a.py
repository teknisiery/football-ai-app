...
# + EV Push (garis bulat), Dynamic EV threshold, BTTS retraining
# ============================================================
...
def calculate_profit(row: dict, stake: float = 100000.0) -> Tuple[float, str]:
    ...
    pred_ou_val = row.get('prediction_ou')
    if pd.notna(pred_ou_val) and pred_ou_val is not None:
        try:
            is_over = int(float(pred_ou_val)) == 1
        except:
            is_over = True
    else:
        pred_str = str(row.get('prediction', '')).strip().upper()
        if pred_str.startswith('OVER'): is_over = True
        elif pred_str.startswith('UNDER'): is_over = False
    ...
    if len(splits) == 1:
        if splits[0][0] % 1 == 0 and total_goals == splits[0][0]:
            return 0.0, "PUSH"
    if total_profit > 0: status = "FULL WIN"
    elif total_profit < 0: status = "FULL LOSE"
    else: status = "PUSH"
...
def compute_detailed_profits(history_df: pd.DataFrame):
    if history_df.empty:
        return 0.0, pd.DataFrame(), pd.DataFrame(), {"total_bets":0,"full_win":0,"half_win":0,"push":0,"half_lose":0,"full_lose":0}, {}
    df = history_df.copy()
    profits, statuses = [], []
    for _, row in df.iterrows():
        p, s = calculate_profit(row.to_dict())
        profits.append(p); statuses.append(s)
    df['profit'] = profits; df['result'] = statuses
    bet_df = df[df['result'] != 'NO BET'].copy()
    total_profit = bet_df['profit'].sum() if not bet_df.empty else 0.0
    if not bet_df.empty:
        sc = bet_df['result'].value_counts()
        summary = {'total_bets':len(bet_df), 'full_win':sc.get('FULL WIN',0), 'half_win':sc.get('HALF WIN',0),
                   'push':sc.get('PUSH',0), 'half_lose':sc.get('HALF LOSE',0), 'full_lose':sc.get('FULL LOSE',0)}
        profit_by_status = bet_df.groupby('result')['profit'].sum().to_dict()
    else:
        summary = {'total_bets':0,'full_win':0,'half_win':0,'push':0,'half_lose':0,'full_lose':0}
        profit_by_status = {}
    if 'settlement_time' in df.columns:
        df_bulan = bet_df.copy()
        df_bulan['settlement_time'] = pd.to_datetime(df_bulan['settlement_time'], errors='coerce')
        df_bulan['bulan'] = df_bulan['settlement_time'].dt.strftime('%Y-%m').fillna('Tanpa Tanggal')
        monthly = {}
        for bulan, grp in df_bulan.groupby('bulan'):
            monthly[bulan] = {
                'profit': grp['profit'].sum(),
                'summary': {'total_bets':len(grp), 'full_win':(grp['result']=='FULL WIN').sum(), 'half_win':(grp['result']=='HALF WIN').sum(),
                            'push':(grp['result']=='PUSH').sum(), 'half_lose':(grp['result']=='HALF LOSE').sum(), 'full_lose':(grp['result']=='FULL LOSE').sum()},
                'df': grp
            }
    else:
        monthly = {}
    return total_profit, bet_df, summary, monthly, profit_by_status

# ============================================================
# RESOURCE REGISTRY, STORAGE, DATABASE MANAGER (sama)
# ============================================================
@dataclass(frozen=True)
class Resource:
    id: str; type: str; serializer: str; default_filename: str; version: int = 1; description: str = ""
...
OPTIONAL_RESOURCES = {ResourceRegistry.HISTORY.id, ResourceRegistry.DATASET.id, ResourceRegistry.DATASET_WITH_GOAL.id,
                      ResourceRegistry.PENDING.id, ResourceRegistry.LEAGUE_STATS.id, ResourceRegistry.THRESHOLD.id,
                      ResourceRegistry.FEATURE_COLUMNS.id, ResourceRegistry.LEAGUE_THRESHOLD.id, ResourceRegistry.LEAGUE_PROFILE.id}
...
class LocalStorageProvider(StorageProvider):
    def __init__(self, base_dir=BASE_DIR): self.base_dir = base_dir
    def _path(self, r): return self.base_dir / r.default_filename
    def load_dataframe(self, r):
        p = self._path(r)
        if not p.exists():
            if r.id in OPTIONAL_RESOURCES: return pd.DataFrame()
            raise FileNotFoundError(f"{r.id} not found")
        return pd.read_csv(p)
    def save_dataframe(self, r, df): df.to_csv(self._path(r), index=False)
    def load_json(self, r):
        p = self._path(r)
        if not p.exists():
            if r.id in OPTIONAL_RESOURCES: return {}
            raise FileNotFoundError(f"{r.id} not found")
...
class GitHubStorageProvider(StorageProvider):
    def __init__(self, owner, repo, branch, token):
        ...
        self.branch = branch; self.token = token
    ...
    def _crud(self, method, r, data=None):
        ...
        if resp.status_code == 409:
            ...
            else: raise RuntimeError("Conflict")
    ...
    def save_pickle(self, r, o):
        buf = BytesIO(); joblib.dump(o, buf); self._crud("put", r, buf.getvalue())
    def exists(self, r):
        url = f"{self.api}/{r.default_filename}?ref={self.branch}"
...
# ============================================================
# CONTRACTS, FEATURE ENGINEERING, LEAGUE PROFILE (sama)
# ============================================================
class PendingContract:
    CORE_COLUMNS = ["match_uid","home_team","away_team","league_name","kickoff_time","prediction","grade","confidence","prediction_id","prediction_time","prediction_status","prediction_version","prediction_source","prediction_notes","home_ht_goals","away_ht_goals","home_goals","away_goals","actual_over","actual_btts","settlement_time","recommendation","stake"]
    @classmethod
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    interactions = [
        ('home_xg','league_avg_goals','xghome_x_leagueavg'), ('away_xg','league_avg_goals','xgaway_x_leagueavg'),
        ('home_xga','league_avg_goals','xgahome_x_leagueavg'), ('away_xga','league_avg_goals','xgaaway_x_leagueavg'),
        ('last5_home_xg','league_avg_goals','last5_home_xg_x_leagueavg'), ('last5_away_xg','league_avg_goals','last5_away_xg_x_leagueavg'),
        ('last5_home_xga','league_avg_goals','last5_home_xga_x_leagueavg'), ('last5_away_xga','league_avg_goals','last5_away_xga_x_leagueavg'),
        ('current_ou','league_avg_goals','ou_line_x_leagueavg'), ('last5_home_over25','league_over25_pct','over25_x_leaguepct'),
...
def update_league_profile(storage: StorageProvider, league_code: int):
    ...
    if existing_name: league_name_final = existing_name
    ...
    else: league_name_final = f'League {league_code}'
    new_row = {'league_code': league_code, 'league_name': league_name_final, 'league_avg_goals': df_league['totalgol_ft'].mean(),
               'league_over25_pct': (df_league['totalgol_ft']>2.5).mean(), 'league_btts_pct': df_league['btts'].mean(),
               'league_under35_pct': (df_league['totalgol_ft']<3.5).mean(), 'eg_p25': df_league['totalgol_ft'].quantile(0.25),
               'eg_p75': df_league['totalgol_ft'].quantile(0.75), 'btts_p25':0.0,'btts_p75':0.0,'ht0_p25':0.0,'ht0_p75':0.0,
    ...
    if "league_profile_cache" in st.session_state: del st.session_state["league_profile_cache"]
...
class PredictionResult:
    expected_goal: float; prob_over: float; prob_under: float; prob_over_raw: float
    ev_over: float; ev_under: float; kelly_over: float; kelly_under: float
    prob_btts: Optional[float]; prob_ht0: Optional[float]
    confidence_ou: float; confidence_btts: Optional[float]; confidence_ht0: Optional[float]
    prediction_ou: int; prediction_btts: int; prediction_ht0: int
    ...
    def predict(self, features_df: pd.DataFrame) -> PredictionResult:
        ...
        for h in range(0, max_goals+1):
            for a in range(0, max_goals+1):
                ...
                if h == 0 and a == 0: prob *= (1 - lam_home * lam_away * rho)
                elif h == 1 and a == 0: prob *= (1 + lam_away * rho)
                elif h == 0 and a == 1: prob *= (1 + lam_home * rho)
        ...
        prob_raw = prob_over_win  # probabilitas menang over (tanpa push)
        prob_over = prob_raw
        if self.calibrator:
            prob_over = np.clip(self.calibrator.predict([[prob_raw]])[0], 0, 1)

        # EV dengan push untuk garis bulat
        if ou_line % 1 == 0:  # garis bulat (2.0, 3.0)
        ...
        return PredictionResult(
            expected_goal=lam, prob_over=prob_over, prob_under=1-prob_over, prob_over_raw=prob_raw,
            ev_over=ev_over, ev_under=ev_under, kelly_over=k_over, kelly_under=k_under,
            prob_btts=prob_btts, prob_ht0=None, confidence_ou=max(prob_over, 1-prob_over),
            confidence_btts=conf_btts, confidence_ht0=None,
            prediction_ou=pred_ou, prediction_btts=pred_btts, prediction_ht0=-1,
            top3_scores=[(int(h), int(a), float(p)) for h, a, p in top3]
        )

# ============================================================
# SESSION MANAGER, UI COMPONENTS (sama)
# ============================================================
@dataclass
class UploadState:
...
class PredictionState:
    ...
    prediction_dataframe: Optional[pd.DataFrame] = None; prediction_result: Optional[dict] = None
...
class SessionManager:
    ...
    def add_debug(self, msg):
        trace = st.session_state.debug_trace; trace.append(msg)
        if len(trace) > 500: st.session_state.debug_trace = trace[-500:]
...
def render_prediction_card(summary: dict):
    ...
    home = safe_html(summary['home']); away = safe_html(summary['away']); league = safe_html(summary['league'])
    ou_pred = safe_html(summary.get('ou_pred','')); ou_line = summary.get('ou_line','')
    rec = safe_html(summary.get('recommendation','')); rec_color = summary.get('rec_color','d')
    over_odds = summary.get('over_odds',0); under_odds = summary.get('under_odds',0)
    ...
    if summary.get('top3_scores'):
        ...
        for i, (h, a, prob) in enumerate(summary['top3_scores']):
            with cols_top[i]:
                st.markdown(f"**{h}‑{a}**"); st.caption(f"{prob*100:.1f}%")
    st.markdown("</div>", unsafe_allow_html=True)

# ============================================================
# PAGES (upload, analysis, pending, settlement, history)
# ============================================================
    ...
    with st.expander("📤 Upload File CSV", expanded=True):
        ...
        if f:
            ...
            if missing:
                ...
                return
            st.session_state.uploaded_file = f; st.session_state.uploaded_df = df
...
def run_analysis(session: SessionManager, storage: StorageProvider, predictor: Predictor):
    ...
    if 'home_team' not in df.columns: df['home_team'] = home
    if 'away_team' not in df.columns: df['away_team'] = away
    df_clean = df.copy()
    if 'league_name' in df_clean.columns: df_clean = df_clean.drop(columns=['league_name'])
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    home, away = st.session_state.uploaded_file.name.replace('.csv','').split(' vs ')
    ou_line = df['current_ou']; over_odds = df['current_over_odds']; under_odds = df['current_under_odds']
    prob_over = r['prob_over']; ev_over = r['ev_over']; ev_under = r['ev_under']
    ...
    return {
        ...
        "confidence_btts": r.get('confidence_btts'), "ht0_pred": "N/A", "insights": [],
...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        home_safe = safe_html(row['home_team']); away_safe = safe_html(row['away_team']); match_uid = row['match_uid']
        pred_str = str(row.get('prediction','')).strip(); rec_str = str(row.get('recommendation','')).strip()
        stake_val = row.get('stake')
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({info})"):
            ...
            init_ht_h = _get_initial(row.get('home_ht_goals')); init_ht_a = _get_initial(row.get('away_ht_goals'))
            init_ft_h = _get_initial(row.get('home_goals')); init_ft_a = _get_initial(row.get('away_goals'))
            c1,c2 = st.columns(2)
            ...
            if st.button("💾 Save Score", key=f"save_{match_uid}", disabled=st.session_state[processing_key]):
                if st.session_state[processing_key]: st.warning("Sedang diproses..."); st.stop()
                st.session_state[processing_key] = True
                ht_h_val = int(ht_h) if ht_h is not None else 0; ht_a_val = int(ht_a) if ht_a is not None else 0
                ft_h_val = int(ft_h) if ft_h is not None else 0; ft_a_val = int(ft_a) if ft_a is not None else 0
                valid, _ = ScoreValidator.validate(row.to_dict(), ht_h_val, ht_a_val, ft_h_val, ft_a_val)
                df.at[idx,'home_ht_goals']=ht_h_val; df.at[idx,'away_ht_goals']=ht_a_val
                df.at[idx,'home_goals']=ft_h_val; df.at[idx,'away_goals']=ft_a_val
                if valid:
                    df.at[idx,'prediction_status']='VALIDATED'
                    full_record = df.loc[idx].to_dict()
                    full_record.update({'home_ht_goals':ht_h_val,'away_ht_goals':ht_a_val,'home_goals':ft_h_val,'away_goals':ft_a_val,
                                        'totalgol_ft':ft_h_val+ft_a_val,'totalgol_ht':ht_h_val+ht_a_val,
                    ...
                    if not existing_hist.empty and 'match_uid' in existing_hist.columns:
                        if full_record['match_uid'] in existing_hist['match_uid'].values:
                            st.warning("Pertandingan sudah ada di History."); st.session_state[processing_key]=False; st.stop()
                    hist_record_df = pd.DataFrame([full_record]); hist_record_df = reorder_columns(hist_record_df)
                    dataset_record_df = hist_record_df.copy()
                    if 'league_name' in dataset_record_df.columns: dataset_record_df.drop(columns=['league_name'], inplace=True)
                    hist = db.load_history(); hist = pd.concat([hist, hist_record_df], ignore_index=True); db.save_history(hist)
                    ...
                    else: db.save_dataset_with_goal(dataset_record_df)
                    df.drop(idx, inplace=True); db.save_pending(df)
                    update_league_profile(storage, int(row.get('league_code',0)))
                    st.success("Skor disimpan dan dipindahkan ke History.")
                else:
                    db.save_pending(df); st.warning("Skor disimpan dengan catatan."); st.session_state[processing_key]=False
                st.rerun()
...
def render_settlement(session: SessionManager, storage: StorageProvider):
    ...
    raw = raw.head(20)
    total_profit = sum(calculate_profit(row.to_dict())[0] for _, row in raw.iterrows())
    st.metric("Total Profit (20 terbaru)", f"Rp {total_profit:+,.0f}")
    for _, row in raw.iterrows():
        home_safe = safe_html(str(row['home_team'])); away_safe = safe_html(str(row['away_team']))
        league_safe = safe_html(str(row.get('league_name',''))); pred_safe = safe_html(str(row.get('prediction','')))
        with st.expander(f"▶ {home_safe} vs {away_safe}"):
            c1,c2 = st.columns(2)
            with c1:
                st.markdown(f"**League:** {league_safe}"); st.markdown(f"**Prediction:** {pred_safe}")
                st.markdown(f"**Score:** {int(row['home_goals'] or 0)}-{int(row['away_goals'] or 0)}")
            with c2:
                profit, status = calculate_profit(row.to_dict())
                color = {"FULL WIN":"#16a34a","HALF WIN":"#65a30d","PUSH":"#6b7280","HALF LOSE":"#f97316","FULL LOSE":"#ef4444","NO BET":"#334155"}.get(status,"#334155")
...
# ============================================================
# EVALUASI & LEARNING (retraining BTTS)
# ============================================================
def evaluate_model(storage: StorageProvider) -> dict:
    ...
    except Exception as e: result={"error":str(e)}; st.session_state[cache_key]=result; return result
    if df.empty: result={"error":"History kosong."}; st.session_state[cache_key]=result; return result
    if 'totalgol_ft' not in df.columns:
        if 'home_goals' in df.columns and 'away_goals' in df.columns: df['totalgol_ft'] = df['home_goals']+df['away_goals']
        else: result={"error":"Data tidak memiliki skor."}; st.session_state[cache_key]=result; return result
    if 'expected_goal' not in df.columns: result={"error":"Data tidak memiliki expected_goal."}; st.session_state[cache_key]=result; return result
    reg_df = df.dropna(subset=['expected_goal','totalgol_ft'])
    if len(reg_df)<2: result={"error":"Data regresi tidak cukup."}; st.session_state[cache_key]=result; return result
    y_true_reg = reg_df['totalgol_ft']; y_pred_reg = reg_df['expected_goal']
    mae = mean_absolute_error(y_true_reg, y_pred_reg); rmse = np.sqrt(mean_squared_error(y_true_reg, y_pred_reg))
    df['total_goals'] = df['home_goals']+df['away_goals']
    df['actual_over'] = np.where(df['total_goals']>df['current_ou'],1,np.where(df['total_goals']<df['current_ou'],0,np.nan))
    class_df = df.dropna(subset=['actual_over','prediction_ou'])
    if len(class_df)<2: accuracy=precision=recall=f1=cm=brier=logloss=None
    else:
        y_true_cls = class_df['actual_over'].astype(int); y_pred_cls = class_df['prediction_ou'].astype(int)
        accuracy=accuracy_score(y_true_cls,y_pred_cls); precision=precision_score(y_true_cls,y_pred_cls,zero_division=0)
        recall=recall_score(y_true_cls,y_pred_cls,zero_division=0); f1=f1_score(y_true_cls,y_pred_cls,zero_division=0)
        cm=confusion_matrix(y_true_cls,y_pred_cls)
        if 'prob_over' in class_df.columns:
            y_true_cal=class_df['actual_over'].astype(int); y_prob=class_df['prob_over']
            brier=brier_score_loss(y_true_cal,y_prob); logloss=log_loss(y_true_cal,y_prob)
        else: brier=logloss=None
    fin_df = df.copy()
    fin_df['profit'], fin_df['status'] = zip(*fin_df.apply(lambda row: calculate_profit(row.to_dict()), axis=1))
    bet_df = fin_df[fin_df['status']!='NO BET']
    if bet_df.empty: win_rate=roi=yield_pct=total_profit=total_bets=None
    else:
        total_bets=len(bet_df); total_profit=bet_df['profit'].sum(); total_stake=total_bets*100000
        roi=total_profit/total_stake*100; yield_pct=total_profit/(total_bets*100000)*100
        full_win=(bet_df['status']=='FULL WIN').sum(); half_win=(bet_df['status']=='HALF WIN').sum()
        win_rate=(full_win+half_win)/total_bets*100
    result = {"mae":mae,"rmse":rmse,"accuracy":accuracy,"precision":precision,"recall":recall,"f1":f1,"cm":cm,
              "brier":brier,"logloss":logloss,"win_rate":win_rate,"roi":roi,"yield_pct":yield_pct,
              "total_bets_fin":total_bets,"total_profit_fin":total_profit,"error":None}
    st.session_state[cache_key]=result; return result
...
def render_learning(session: SessionManager, storage: StorageProvider):
    ...
    c1,c2=st.columns(2); c1.metric("📏 MAE",f"{eval_result['mae']:.2f}"); c2.metric("📐 RMSE",f"{eval_result['rmse']:.2f}")
    st.markdown("**Klasifikasi Over/Under**")
    if eval_result['accuracy'] is not None:
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Akurasi",f"{eval_result['accuracy']:.2%}"); c2.metric("Precision",f"{eval_result['precision']:.2%}")
        c3.metric("Recall",f"{eval_result['recall']:.2%}"); c4.metric("F1‑Score",f"{eval_result['f1']:.2%}")
    ...
    if eval_result['brier'] is not None:
        c1,c2=st.columns(2); c1.metric("Brier Score",f"{eval_result['brier']:.4f}"); c2.metric("Log Loss",f"{eval_result['logloss']:.4f}")
    else: st.info("Data kalibrasi tidak tersedia.")
    st.markdown("**Finansial (hanya taruhan yang direkomendasikan)**")
    if eval_result['win_rate'] is not None:
        c1,c2,c3=st.columns(3)
        c1.metric("Win Rate",f"{eval_result['win_rate']:.2f}%"); c2.metric("ROI",f"{eval_result['roi']:.2f}%"); c3.metric("Yield",f"{eval_result['yield_pct']:.2f}%")
...
# ============================================================
# REMAINING PAGES & MAIN APP
# ============================================================
def render_database(session, storage):
...
def render_performance(session, app_storage, db_storage):
...
def main():
    ...
    with st.sidebar:
        ...
        st.title(APP_TITLE); st.caption(f"v{APP_VERSION}")
        ev_over, ev_under = load_ev_thresholds(db_storage)
        st.metric("Threshold Over", f"{ev_over:.2f}"); st.metric("Threshold Under", f"{ev_under:.2f}")
        ...
        st.divider()
        try: hist_df = db_storage.load_dataframe(ResourceRegistry.HISTORY)
        ...
        cols = st.columns(6)
        cols[0].metric("Total Bets", summary['total_bets']); cols[1].metric("Full Win", summary['full_win'])
        cols[2].metric("Half Win", summary['half_win']); cols[3].metric("Push", summary['push'])
        cols[4].metric("Half Lose", summary['half_lose']); cols[5].metric("Full Lose", summary['full_lose'])
        ...
        if monthly_data:
            ...
            for bulan in sorted(monthly_data.keys(), reverse=True):
                ...
                with st.expander(f"{bulan} - Rp {data['profit']:+,.0f}"):
                    ...
                    cols_b = st.columns(6)
                    cols_b[0].metric("Total Bets", bulan_sum['total_bets']); cols_b[1].metric("Full Win", bulan_sum['full_win'])
                    cols_b[2].metric("Half Win", bulan_sum['half_win']); cols_b[3].metric("Push", bulan_sum['push'])
                    cols_b[4].metric("Half Lose", bulan_sum['half_lose']); cols_b[5].metric("Full Lose", bulan_sum['full_lose'])
        ...
        else: st.caption("Belum ada data profit bulanan.")
    ...
    if 'uploaded_file' not in st.session_state:
        ...
        if st.button("🔄 Ganti File"):
            del st.session_state.uploaded_file; session.set_prediction_state(PredictionState())
            st.session_state.analysis_needed = False; st.rerun()
        if predictor is None:
            st.warning("Model tidak ditemukan. Silakan latih model di tab Learning.")
        else:
            if st.session_state.get('analysis_needed', False):
                st.session_state.analysis_needed = False
                with st.spinner("Menganalisis..."): run_analysis(session, db_storage, predictor)
                st.rerun()
            if not session.get_prediction_state().processed:
                if st.button("🚀 ANALYZE MATCH", type="primary"): run_analysis(session, db_storage, predictor); st.rerun()
        if session.get_prediction_state().processed:
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ps = session.get_prediction_state()
                    row = ps.prediction_dataframe.iloc[0].to_dict(); row.update(ps.prediction_result)
                    ...
                    row['prediction_status'] = 'PENDING'; row['home_team'] = summary['home']; row['away_team'] = summary['away']
                    row['prediction'] = f"{summary['ou_pred']} {summary['ou_line']}"
                    row['recommendation'] = summary['recommendation']; row['stake'] = summary['stake']
                    ...
                    if not existing_hist.empty and 'match_uid' in existing_hist.columns:
                        if row['match_uid'] in existing_hist['match_uid'].values:
                            st.warning("Pertandingan ini sudah ada di History."); st.stop()
                    ...
                    if not pend.empty and row['match_uid'] in pend['match_uid'].values:
                        ...
                        db.save_pending(pend); st.success("Disimpan ke Pending!")
                    st.rerun()
    ...
    if st.session_state.get("debug_trace"):
        with st.expander("📜 Raw Debug Trace"):
            for line in st.session_state.debug_trace[-100:]: st.text(line)
