...
def render_settlement():
    ...
    if 'settlement_time' in history.columns:
        history = history.sort_values('settlement_time', ascending=False)
    elif 'kickoff_time' in history.columns:
        history = history.sort_values('kickoff_time', ascending=False)
    history = history.head(20)

    # Tampilkan ringkasan total profit
...
def render_history_manager():
    ...
    # DEBUG: coba baca langsung dari storage
    try:
    ...
    try:
        ...
        st.error(f"Gagal load_history: {e}")
        return
    
    if history.empty:
        st.warning("History kosong setelah normalize.")
        return
    
    # Urutkan berdasarkan settlement_time (terbaru di atas)
    if 'settlement_time' in history.columns:
        history = history.sort_values('settlement_time', ascending=False)
    elif 'kickoff_time' in history.columns:
        history = history.sort_values('kickoff_time', ascending=False)
