...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    if prob_btts is not None and 0 < prob_btts < 1:
        ...
        # Ambil odds BTTS dari file unggahan (current)
        if session.btts_odds:
            ...
            with st.expander(f"▶ {home_safe} vs {away_safe}"):
                ...
                if st.session_state.get(f"processing_{match_uid}", False):
                    ...
                    if valid:
                        ...
                        # Gunakan baris terbaru dari DataFrame, bukan row hasil iterrows
                    ...
                    if mask.any():
                        ...
                        with st.expander("⚽ Distribusi Skor Kombinasi"):
                            ...
                            for cat in categories:
                                prob = current_dist.get(cat, 0.0)
                                est_count = int(round(prob * current_total)) if current_total else 0
