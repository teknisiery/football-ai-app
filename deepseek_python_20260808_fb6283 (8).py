...
def main():
    ...
    with st.sidebar:
        ...
        total_profit, profit_df, summary_raw, monthly_data, profit_by_status = compute_detailed_profits(hist_df)
        # Tangani cache lama yang mungkin mengembalikan DataFrame (bahkan DataFrame kosong)
        if isinstance(summary_raw, pd.DataFrame):
            if not summary_raw.empty:
                summary = summary_raw.iloc[0].to_dict()
            else:
                summary = {
        ...
        if monthly_data:
            ...
            for bulan_key in sorted(monthly_data.keys(), reverse=True):
                ...
                if bulan_key == 'Tanpa Tanggal':
                    ...
                    # Pastikan bisa di-split; jika tidak, gunakan key sebagai label
                    parts = bulan_key.split('-')
                    if len(parts) == 2:
                        year, month = parts
                        label_bulan = f"{bulan_map.get(month, month)} {year}"
                    else:
                        label_bulan = bulan_key  # fallback
