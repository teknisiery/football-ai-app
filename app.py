...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    # History fitur 1X2: hanya 7 hari terakhir (hari ini + 6 hari sebelumnya).
    hari_ini = datetime.now().date()
    start_date = hari_ini - timedelta(days=6)
    ...
    if df.empty:
        st.info("Data profit 1X2 belum tersedia untuk periode ini.")
        return
...
def render_btts_history(session: SessionManager, storage: StorageProvider):
    ...
    # History fitur BTTS: hanya 7 hari terakhir.
    hari_ini = datetime.now().date()
    start_date = hari_ini - timedelta(days=6)
    df = df[(df['tanggal'] >= start_date) & (df['tanggal'] <= hari_ini)].copy()
    if df.empty:
    ...
    df = df[valid_profit].copy()
    if df.empty:
        st.info("Data profit BTTS belum tersedia untuk periode ini.")
        return
