# ============================================================
# FOOTBALL AI V2 STABLE REV24 – Match UID for Dataset Updates
# ============================================================
...
def _append_dataset(df_existing: pd.DataFrame, new_record: pd.DataFrame, dedup: bool = True) -> pd.DataFrame:
    ...
    if dedup:
        # Hapus baris dengan identitas sama dari existing
...
def append_raw_dataset(uploaded_df: pd.DataFrame):
    """Append validated match to dataset_ou.csv (raw)."""
    db = DatabaseManager()
    try:
    ...
    updated = _append_dataset(existing, uploaded_df, dedup=True)
    db.save_dataset(updated)

def append_training_dataset(uploaded_df: pd.DataFrame, home_team: str, away_team: str):
    """Copy raw data, tambah kolom goal dan match_uid, simpan ke dataset_withgoal.csv."""
    df = uploaded_df.copy()
    # Buat match_uid dari nama tim (dari nama file) dan kickoff_time
    kickoff = str(df["kickoff_time"].iloc[0]) if "kickoff_time" in df.columns else ""
    match_uid = generate_match_uid(home_team, away_team, kickoff)
    df["match_uid"] = match_uid
...
def update_dataset_goals(match_uid: str, home_ht: int, away_ht: int, home_ft: int, away_ft: int):
    """Update goal columns di dataset_withgoal.csv menggunakan match_uid."""
    db = DatabaseManager()
    try:
    ...
    if dataset.empty or "match_uid" not in dataset.columns:
        return

    mask = dataset["match_uid"] == match_uid
    if mask.any():
        idx = dataset[mask].index[0]
...
def save_pending_score(idx, ht_home, ht_away, ft_home, ft_away, pending_all, db):
    ...
    if valid:
        ...
        # Update dataset_withgoal.csv menggunakan match_uid
        match_uid = generate_match_uid(
            str(original.get("home_team", "")),
            str(original.get("away_team", "")),
            str(original.get("kickoff_time", ""))
        )
        update_dataset_goals(match_uid, ht_home, ht_away, ft_home, ft_away)
    ...
    def run_analysis(self, uploaded_file) -> dict:
        ...
        if context.get("validation_success",False):
            ...
            try:
                ...
                append_raw_dataset(upload_df)
                append_training_dataset(upload_df, home, away)   # kirim nama tim
