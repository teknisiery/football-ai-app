...
from services.match_pnl import build_match_pnl, apply_1x2_odds_floor, ONE_X_TWO_MIN_ODDS, format_pnl_table_html, visible_match_dates
...
def render_match_pnl(session: SessionManager, storage: StorageProvider):
    """Render MATCH P&L as lightweight daily groups with on-demand details."""
    ...
    # match_uid adalah identitas pertandingan; record terbaru menangani duplikasi legacy.
    df = df.drop_duplicates(subset=["match_uid"], keep="first")

    # Hitung ledger sekali untuk semua history agar angka akumulasi tetap konsisten.
    results = []
    for _, row in df.iterrows():
        try:
            result = build_match_pnl(row.to_dict())
            result["settlement_dt"] = row.get("_match_pnl_dt")
    ...
    total_stake = sum(float(r.get("total_stake", 0.0) or 0.0) for r in results)
    total_return = sum(float(r.get("total_return", 0.0) or 0.0) for r in results)
    total_net = total_return - total_stake
    st.metric("Total Modal", f"Rp {total_stake:,.0f}")
    st.metric("Total Return", f"Rp {total_return:,.0f}")
    st.metric("Net P/L", f"Rp {total_net:+,.0f}")
    ...
    # UI hanya menampilkan 7 hari terakhir; history lebih lama tetap tersimpan dan
    # tetap masuk ke akumulasi total di atas.
    visible = {}
    for result in results:
        dt = result.get("settlement_dt")
        if pd.isna(dt):
            continue
        tgl = pd.Timestamp(dt).date()
        if start_date <= tgl <= hari_ini:
            visible.setdefault(tgl, []).append(result)

    def day_label(tgl):
        if tgl == hari_ini:
            return "Hari Ini"
        if tgl == hari_ini - timedelta(days=1):
            return "Kemarin"
        return f"{hari_map[tgl.weekday()]}, {tgl.day} {bulan_map[tgl.month]} {tgl.year}"

    for offset, tgl in enumerate(visible_match_dates(hari_ini)):
        group = sorted(
            visible.get(tgl, []),
            key=lambda r: pd.Timestamp(r.get("settlement_dt")) if not pd.isna(r.get("settlement_dt")) else pd.Timestamp.min,
            reverse=True,
        )
        day_profit = sum(float(r.get("net_pnl", 0.0) or 0.0) for r in group)
        label = f"{day_label(tgl)} — Rp {day_profit:+,.0f}"

        # Hari ini dibuka secara default, hari lain tersembunyi seperti Settlement/1X2/BTTS.
        with st.expander(label, expanded=(offset == 0)):
            if not group:
                st.caption("Tidak ada pertandingan dengan taruhan pada hari ini.")
                continue

            # Ringkasan full-color: ringan karena hanya HTML, tanpa membuat detail
            # expander untuk ratusan pertandingan.
            options = []
            option_to_result = {}
            for idx, result in enumerate(group):
                ...
                net = float(result.get("net_pnl", 0.0) or 0.0)
                sign = "+" if net > 0 else ""
                summary = f"{home} vs {away} | {score} | Rp {sign}{net:,.0f}"
                options.append(summary)
                option_to_result[summary] = result
                ...
                st.markdown(
                    ...
                    f"border-radius:9px;margin:5px 0;font-weight:700;'>"
                    f"{home} vs {away} | {score} | Rp {sign}{net:,.0f}</div>",
                    unsafe_allow_html=True,
                )

            # Satu selector per hari: detail hanya dihitung/render untuk satu match,
            # sehingga tidak lagi membuat ratusan tabel HTML saat halaman dibuka.
            selector_key = f"match_pnl_select_{tgl.isoformat()}"
            selected_summary = st.selectbox(
                "Pilih pertandingan untuk melihat rincian P&L",
                options,
                key=selector_key,
                label_visibility="collapsed",
            )
            selected = option_to_result[selected_summary]

            st.markdown("---")
            st.markdown("**Total Modal**")
            st.markdown(f"### Rp {selected['total_stake']:,.0f}")
            if selected["legs"]:
                st.markdown(format_pnl_table_html(selected["legs"]), unsafe_allow_html=True)
            else:
                st.info("Tidak ada taruhan aktif pada pertandingan ini.")

            net = float(selected["net_pnl"])
            if net > 0:
                net_color = "#16a34a"
            elif net < 0:
                net_color = "#ef4444"
                ...
                net_color = "#94a3b8"
            st.markdown("---")
            st.markdown(f"**TOTAL MODAL**  \nRp {selected['total_stake']:,.0f}")
            st.markdown(f"**TOTAL RETURN**  \nRp {selected['total_return']:,.0f}")
            st.markdown(
