...
class PendingContract:
    CORE_COLUMNS = [
        ...
        "btts_decision_reason", "btts_low_odds_protection",
        "btts_opposite_ev_required"
    ]
    @classmethod
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
        ...
        if prob_for_ev and odds_1x2_dict:
            ...
            if ev_list:
                ...
                if market_odds and market_odds > 1:
                    stake_1x2 = 100000.0 / (market_odds - 1)
                else:
                    stake_1x2 = 0.0
            else:
    ...
    btts_decision_reason = None
    btts_low_odds_protection = False
    btts_opposite_ev_required = 0.30

    # LOW-ODDS PROTECTION BTTS:
    # - Odds < 1.35 cannot be bet directly.
    # - If YES is <1.35, only NO may be bet when EV(NO) > +0.30.
    # - If NO is <1.35, only YES may be bet when EV(YES) > +0.30.
    # - Otherwise NO BET. Raw probabilities/odds/EV are still stored.
    ...
    if prob_btts is not None and 0 < prob_btts < 1:
        ...
        if market_odds_dict:
            ...
            yes_low = market_odds_btts_yes is not None and market_odds_btts_yes < 1.35
            no_low = market_odds_btts_no is not None and market_odds_btts_no < 1.35
            btts_low_odds_protection = bool(yes_low or no_low)

            if yes_low:
                if ev_btts_no is not None and ev_btts_no > btts_opposite_ev_required:
                    recommendation_btts = 'NO'
                    btts_decision_reason = 'LOW_ODDS_YES: opposite NO EV > +0.30'
                else:
                    recommendation_btts = 'NO BET'
                    btts_decision_reason = 'LOW_ODDS_YES: opposite NO EV <= +0.30'
            elif no_low:
                if ev_btts_yes is not None and ev_btts_yes > btts_opposite_ev_required:
                    recommendation_btts = 'YES'
                    btts_decision_reason = 'LOW_ODDS_NO: opposite YES EV > +0.30'
                else:
                    recommendation_btts = 'NO BET'
                    btts_decision_reason = 'LOW_ODDS_NO: opposite YES EV <= +0.30'
            else:
                # Existing value rule is preserved when both sides are >= 1.35.
                ev_list_btts = []
                if ev_btts_yes is not None and ev_btts_yes > ev_th_over:
                    ev_list_btts.append(('YES', ev_btts_yes))
                if ev_btts_no is not None and ev_btts_no > ev_th_over:
                    ev_list_btts.append(('NO', ev_btts_no))
                if ev_list_btts:
                    best_btts, best_ev_btts = max(ev_list_btts, key=lambda x: x[1])
                    recommendation_btts = best_btts
                    btts_decision_reason = 'NORMAL_VALUE_RULE'
                else:
                    recommendation_btts = 'NO BET'
                    btts_decision_reason = 'NO_POSITIVE_EV'
            ...
            if recommendation_btts in ('YES', 'NO'):
                ...
                if odds_for_stake and odds_for_stake > 1:
                    ...
                    stake_btts = 0.0
                    recommendation_btts = 'NO BET'
                    btts_decision_reason = 'INVALID_MARKET_ODDS'
        else:
            recommendation_btts = None
            btts_decision_reason = 'MARKET_ODDS_MISSING'
    else:
        prob_btts = None
    ...
    return {
        ...
        "btts_decision_reason": btts_decision_reason,
        "btts_low_odds_protection": btts_low_odds_protection,
        "btts_opposite_ev_required": btts_opposite_ev_required
    }
...
def _pnl_bet_row(label, stake, odds, status):
    stake = float(stake or 0)
    odds = float(odds or 0)
    status = str(status or 'NO BET').upper()
    if stake <= 0:
        return None
    if status == 'WIN':
        pnl = stake * (odds - 1.0)
        ...
        pnl = -stake
        ret = 0.0
    return {'label': label, 'stake': stake, 'odds': odds, 'status': status, 'pnl': pnl, 'return': ret}


def _settle_ou_for_match(row):
    """Return OU status using the same ProfitCalculator semantics as Settlement Audit."""
    try:
        pc = ProfitCalculator()
        profit, status = pc.calculate(row.to_dict(), 100000)
        # ProfitCalculator's fixed 100k argument is the target profit convention;
        # for MATCH P&L we need the actual row stake and market odds.
        pred = str(row.get('prediction', '') or '').upper()
        if pred.startswith('OVER'):
            odds = float(row.get('current_over_odds', 0) or 0)
        elif pred.startswith('UNDER'):
            odds = float(row.get('current_under_odds', 0) or 0)
        else:
            return 'NO BET', 0.0, 0.0
        stake = float(row.get('stake', 0) or 0)
        if stake <= 0 or odds <= 1:
            return 'NO BET', 0.0, 0.0
        if status == 'FULL WIN':
            return 'WIN', stake * odds, stake * (odds - 1.0)
        if status == 'PUSH':
            return 'PUSH', stake, 0.0
        if status == 'HALF WIN':
            return 'HALF WIN', stake + stake * (odds - 1.0) * 0.5, stake * (odds - 1.0) * 0.5
        if status == 'HALF LOSE':
            return 'HALF LOSE', stake * 0.5, -stake * 0.5
        return 'LOSE', 0.0, -stake
    except Exception:
        return 'NO BET', 0.0, 0.0
...
def render_match_pnl(session: SessionManager, storage: StorageProvider):
    ...
    try:
        ...
        raw = pd.DataFrame()
    if raw.empty:
        st.info("Belum ada pertandingan di History.")
        return

    required = ['settlement_time', 'home_team', 'away_team']
    missing = [c for c in required if c not in raw.columns]
    if missing:
        st.warning(f"Kolom history belum lengkap: {', '.join(missing)}")
        return
    ...
    for _, row in raw.iterrows():
        ...
        # OU
        if float(row.get('stake', 0) or 0) > 0:
            status, ret, pnl = _settle_ou_for_match(row)
            if status != 'NO BET':
                pred = str(row.get('prediction', '') or '').upper()
                odds = float(row.get('current_over_odds', 0) if pred.startswith('OVER') else row.get('current_under_odds', 0) or 0)
                bets.append(_pnl_bet_row(pred, row.get('stake', 0), odds, status))

        # 1X2
        pred = str(row.get('prediction_1x2', '') or '').strip().upper()
        stake = float(row.get('stake_1x2', 0) or 0)
        if pred in ('HOME', 'DRAW', 'AWAY') and stake > 0:
            ...
            odds = float(row.get(f'market_odds_1x2_{pred.lower()}', 0) or 0)
            bets.append(_pnl_bet_row(f'1X2 {pred}', stake, odds, 'WIN' if pred == actual else 'LOSE'))

        # BTTS
        pred = str(row.get('recommendation_btts', '') or '').strip().upper()
        stake = float(row.get('stake_btts', 0) or 0)
        if pred in ('YES', 'NO') and stake > 0:
            actual_btts = 'YES' if int(float(row.get('home_goals', 0) or 0)) > 0 and int(float(row.get('away_goals', 0) or 0)) > 0 else 'NO'
            odds_col = 'market_odds_btts_yes' if pred == 'YES' else 'market_odds_btts_no'
            odds = float(row.get(odds_col, 0) or 0)
            bets.append(_pnl_bet_row(f'BTTS {pred}', stake, odds, 'WIN' if pred == actual_btts else 'LOSE'))

        # Correct Score — each of the three bets is settled independently.
        hg = int(float(row.get('home_goals', 0) or 0))
        ag = int(float(row.get('away_goals', 0) or 0))
        for i in range(1, 4):
            score = str(row.get(f'cs_score_{i}', '') or '').strip()
            stake = float(row.get(f'cs_stake_{i}', 0) or 0)
            odds = float(row.get(f'cs_odds_{i}', 0) or 0)
            if not score or stake <= 0 or odds <= 1:
                continue
            try:
                sh, sa = map(int, score.split(':'))
            except Exception:
                continue
            bets.append(_pnl_bet_row(f'CS {score}', stake, odds, 'WIN' if (sh == hg and sa == ag) else 'LOSE'))
        ...
        total_return = sum(b['return'] for b in bets)
        net = total_return - total_stake
        settlement = pd.to_datetime(row.get('settlement_time'), errors='coerce')
        rows.append({
            'idx': _,
            'settlement_time': settlement,
            'home': str(row.get('home_team', '')),
            'away': str(row.get('away_team', '')),
            'score': f"{int(float(row.get('home_goals', 0) or 0))}-{int(float(row.get('away_goals', 0) or 0))}",
            'bets': bets,
    ...
    if not rows:
        st.info("Belum ada pertandingan dengan stake yang dapat dihitung.")
        return

    rows.sort(key=lambda x: x['settlement_time'] if pd.notna(x['settlement_time']) else pd.Timestamp.min, reverse=True)
    total_stake_all = sum(x['total_stake'] for x in rows)
    total_return_all = sum(x['total_return'] for x in rows)
    net_all = total_return_all - total_stake_all

    c1, c2, c3 = st.columns(3)
    c1.metric("Total Modal", f"Rp {total_stake_all:,.0f}")
    c2.metric("Total Return", f"Rp {total_return_all:,.0f}")
    c3.metric("Net P/L", f"Rp {net_all:+,.0f}")
    st.caption(f"{len(rows)} pertandingan dengan minimal satu taruhan. Setiap pertandingan dihitung sebagai satu portfolio P&L.")

    for item in rows:
        net = item['net']
        icon = '🟢' if net > 0 else ('🔴' if net < 0 else '⚪')
        title = f"{icon} {item['home']} vs {item['away']} | {item['score']} | Rp {net:+,.0f}"
        with st.expander(title):
            st.markdown(f"**Total Modal**  \nRp {item['total_stake']:,.0f}")
            st.divider()
            for bet in item['bets']:
                st.markdown(
                    f"**{bet['label']}**  \n"
                    f"Modal     Rp {bet['stake']:,.0f}  \n"
                    f"Odds      {bet['odds']:.2f}  \n"
                    f"Hasil     {bet['status']}  \n"
                    f"P/L       Rp {bet['pnl']:+,.0f}"
                )
                st.markdown("---")
            st.markdown(f"**TOTAL MODAL**  \nRp {item['total_stake']:,.0f}")
            st.markdown(f"**TOTAL RETURN**  \nRp {item['total_return']:,.0f}")
            st.markdown(f"### NET P/L: Rp {item['net']:+,.0f}")
...
def main():
    ...
    if session.uploaded_file is None:
        ...
        if session.get_prediction_state().processed:
            ...
            if combined_file:
                try:
                    raw_odds = combined_file.getvalue()
                    file_hash = hashlib.sha256(raw_odds).hexdigest()
                    combined = parse_combined_odds_csv(raw_odds)

                    # Always show an explicit parser result. Never fail silently.
                    if combined.get('1x2'):
                        session.uploaded_odds_1x2 = combined['1x2']
                        session.uploaded_open_1x2 = combined.get('open_1x2')
                        st.success(
                            f"✅ Odds 1X2 terbaca — Current: "
                            f"Home {combined['1x2']['home']:.2f} | "
                            f"Draw {combined['1x2']['draw']:.2f} | "
                            f"Away {combined['1x2']['away']:.2f}"
                        )
                    else:
                        session.uploaded_odds_1x2 = None
                        session.uploaded_open_1x2 = None
                        st.error("❌ Odds 1X2 tidak ditemukan di file.")

                    if combined.get('cs'):
                        session.uploaded_odds = combined['cs']
                        st.success(f"✅ Correct Score terbaca — {len(combined['cs'])} odds.")
                    else:
                        session.uploaded_odds = None
                        st.error("❌ Correct Score tidak ditemukan di file.")

                    for warning in combined.get('warnings', []):
                        st.caption(f"ℹ️ {warning}")

                    if combined.get('errors'):
                        for err in combined['errors']:
                            # Do not turn a partial success into a generic failure.
                            if not (("Odds 1X2 tidak ditemukan" in err and combined.get('1x2')) or
                                    ("Odds Correct Score tidak ditemukan" in err and combined.get('cs'))):
                                st.warning(f"⚠️ {err}")

                    # Persist only once per exact uploaded file. This prevents
                    # Streamlit reruns from duplicating CS odds history.
                    processed_hash = st.session_state.get('_combined_odds_hash')
                    if file_hash != processed_hash and (combined.get('1x2') or combined.get('cs')):
                        ps = session.get_prediction_state()
                        match_uid = ps.prediction_dataframe.iloc[0]['match_uid']

                        # Build CS history row from the CS section when available.
                        row_data = {}
                        if combined.get('cs'):
                            cs_lines_start = raw_odds.decode('utf-8-sig', errors='replace')
                            marker = cs_lines_start.lower().find('type,score,odds')
                            cs_only = raw_odds if marker < 0 else cs_lines_start[marker:].encode('utf-8')
                            row_data = convert_odds_to_wide(match_uid, cs_only)
                        row_data['match_uid'] = match_uid
                        ...
                        if combined.get('open_1x2') and combined.get('1x2'):
                            for k in ['home', 'draw', 'away']:
                                ov = combined['open_1x2'].get(k)
                                cv = combined['1x2'].get(k)
                                row_data[f'movement_1x2_{k}'] = round(cv - ov, 4) if ov is not None and cv is not None else np.nan
                        ...
                        if 'match_uid' in cs_hist.columns and match_uid in cs_hist['match_uid'].values:
                            idx_cs = cs_hist.index[cs_hist['match_uid'] == match_uid][0]
                            for col, value in row_data.items():
                                cs_hist.at[idx_cs, col] = value
                        else:
                        ...
                        st.session_state['_combined_odds_hash'] = file_hash
                        st.success("💾 Odds berhasil disimpan ke history.")

                except Exception as e:
                    st.error(f"❌ Gagal memproses file odds: {type(e).__name__}: {e}")
            else:
                # Do not erase previously parsed odds during a Streamlit rerun.
                # State is cleared only by the explicit 'Ganti File' workflow.
                pass
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ...
                    row['btts_decision_reason'] = summary.get('btts_decision_reason')
                    row['btts_low_odds_protection'] = summary.get('btts_low_odds_protection', False)
                    row['btts_opposite_ev_required'] = summary.get('btts_opposite_ev_required', 0.30)
        ...
        tab1, tab2, tab_pnl, tab2_5, tab2_6, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
            "📋 Pending",
            "📝 Settlement",
            "💰 MATCH P&L",
            "🎯 1X2",
        ...
        with tab_pnl: render_match_pnl(session, db_storage)
