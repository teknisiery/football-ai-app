...
def update_league_profile(storage: StorageProvider, league_code: int, session: 'SessionManager' = None):
    ...
    new_row = {
        ...
        'btts_threshold': 0.22,
        # --- 1X2: ambil dari profil yang sudah ada, jika tidak ada isi default ---
...
class Predictor:
    ...
    def predict(self, features_df: pd.DataFrame) -> PredictionResult:
        ...
        # --- 1X2: hitung prob model dari score_probs ---
...
def render_prediction_card(summary: dict):
    ...
    if summary.get('prob_1x2_model'):
        ...
        col_m, col_l, col_h = st.columns(3)
        with col_m:
            st.markdown("**Model**")
            st.write(f"Home: {model.get('home',0):.1%}")
            st.write(f"Draw: {model.get('draw',0):.1%}")
            st.write(f"Away: {model.get('away',0):.1%}")
        with col_l:
            st.markdown("**Liga**")
            st.write(f"Home: {league.get('home',0):.1%}")
            st.write(f"Draw: {league.get('draw',0):.1%}")
            st.write(f"Away: {league.get('away',0):.1%}")
        with col_h:
            st.markdown("**Hybrid**")
            st.write(f"Home: {hybrid_final.get('home',0):.1%}")
            st.write(f"Draw: {hybrid_final.get('draw',0):.1%}")
            st.write(f"Away: {hybrid_final.get('away',0):.1%}")
