...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    if odds_1x2_dict:
        # Hitung fair probability 1X2 secara manual (hindari bug calculate_fair_probs)
        implied = {}
        ...
        if total_implied > 0:
            ...
            # Gunakan fair_1x2 sebagai 'pasar'
        ...
        # Perhitungan EV dan rekomendasi
...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    # Hanya ambil baris yang memiliki prediction_1x2 dan bukan NO BET
    ...
    for idx, row in df.iterrows():
        ...
        if pred and pred != 'NO BET' and stake > 0:
            ...
            if market_odds > 0:
                ...
                df.at[idx, 'profit_1x2'] = 0.0
                df.at[idx, 'result_1x2'] = 'N/A'
        else:
            df.at[idx, 'profit_1x2'] = 0.0
            df.at[idx, 'result_1x2'] = 'N/A'

    st.metric("Total Profit 1X2", f"Rp {total_profit:+,.0f}")
    st.metric("Win Rate", f"{win_count}/{total_bets} ({win_count/total_bets*100:.1f}%)" if total_bets > 0 else "N/A")

    # Tampilkan seperti settlement
    for _, row in df.iterrows():
        ...
        if result == 'WIN':
            ...
            bg = "#334155"
        profit_str = f"Rp {profit:+,.0f}" if profit != 0 else "Rp 0"
        st.markdown(
            f"<div style='background-color:{bg}; padding:8px; border-radius:8px; "
            f"margin:4px 0; color:white;'>"
            f"<strong>{home} vs {away}</strong> | {league} | Pred: {pred} | "
            f"Skor: {score} | {result} | {profit_str}"
            f"</div>", unsafe_allow_html=True
        )
