...
# + Fix: NO BET tidak dihitung sebagai taruhan
# ============================================================
...
def calculate_profit(row: dict, stake: float = 100000.0) -> Tuple[float, str]:
    ...
    """
    # 1. Periksa apakah ini taruhan yang benar-benar dipasang
    ...
    # 2. Tentukan stake yang digunakan (default 100000 jika tidak ada)
...
def compute_detailed_profits(history_df: pd.DataFrame):
    """
    Menghitung profit hanya untuk taruhan yang benar‑benar dipasang (stake > 0, bukan NO BET).
    """
    if history_df.empty:
    ...
    # Hanya pertandingan dengan taruhan yang dipertimbangkan untuk statistik
...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({row['prediction_status']})"):
            ...
            save_key = f"save_disabled_{idx}"
            if save_key not in st.session_state:
                st.session_state[save_key] = False

            if st.button("💾 Save Score", key=f"save{idx}", disabled=st.session_state[save_key]):
