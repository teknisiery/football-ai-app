...
def _match_identifier(row: pd.Series) -> Tuple[str, str, str, str]:
    """Stable identity: league_code, kickoff_time, home_team, away_team."""
    return (
...
def _append_dataset(df_existing: pd.DataFrame, new_record: pd.DataFrame, dedup: bool = True) -> pd.DataFrame:
    """Append or replace record based on match identity."""
    ...
    if dedup:
        ...
        combined = pd.concat([df_filtered, new_record], ignore_index=True)
    else:
...
def _reorder_columns(df: pd.DataFrame, extra_cols: list = None) -> pd.DataFrame:
    """
    Urutan yang diinginkan: league_code, home_team, away_team, kickoff_time, lalu kolom asli lainnya,
    kemudian extra_cols di akhir (jika ada).
    """
    # Kolom inti yang harus di depan
    front_cols = ["league_code", "home_team", "away_team", "kickoff_time"]
    # Kolom asli selain front_cols
    other_cols = [c for c in df.columns if c not in front_cols and c not in (extra_cols or [])]
    # Jika ada extra_cols, letakkan di akhir
...
def append_raw_dataset(uploaded_df: pd.DataFrame, home_team: str, away_team: str):
    """Append validated match to dataset_ou.csv (raw), dengan kolom home_team/away_team dan urutan yang benar."""
    df = uploaded_df.copy()
    df["home_team"] = home_team
    df["away_team"] = away_team
    df = _reorder_columns(df)  # urutkan sesuai spesifikasi

    db = DatabaseManager()
    try:
        existing = db.load_dataset()
    except:
...
def append_training_dataset(uploaded_df: pd.DataFrame, home_team: str, away_team: str):
    """Copy raw data, tambah kolom team, goal, urutkan, simpan ke dataset_withgoal.csv."""
    df = uploaded_df.copy()
    df["home_team"] = home_team
    df["away_team"] = away_team
    # Kolom gol diinisialisasi NaN
    goal_cols = ["home_ht_goals", "away_ht_goals", "home_goals", "away_goals"]
    for col in goal_cols:
        df[col] = np.nan
    df = _reorder_columns(df, extra_cols=goal_cols)

    db = DatabaseManager()
    try:
        existing = db.load_dataset_with_goal()
    except:
        existing = pd.DataFrame()
...
def update_dataset_goals(match_id: Tuple[str, str, str, str],
    ...
    """Update goal columns di dataset_withgoal.csv menggunakan identitas: league_code, kickoff_time, home_team, away_team."""
    db = DatabaseManager()
    try:
    ...
    # Cocokkan berdasarkan 4 kolom identitas
    match_condition = (
...
def calculate_decision_engine(feat_row, pred_row, ev_thresholds, league_thresholds_df, historical_performance):
    ...
    # Edge (must be computed here, used by both Market Brain and Decision Engine)
    ...
    # - Debug
    #
    # Display Metrics MUST NEVER affect:
    # - Grade
    # - Recommendation
    # - Decision
    # ==================================================
    ...
    # Market Brain (uses edge and confidence)
    ...
    # - Confidence
    #
    # Decision Engine MUST NEVER use:
    # - Football Brain
    # - League Brain
    # - Market Brain
    # - AI Score
    ...
    if ev_signal == "avoid" and (ev_over > 0 or ev_under > 0):
        ev_signal = "marginal"
...
def _settlement_result(row):
    """Menghitung WIN/LOSE/PUSH berdasarkan prediksi over/under dan hasil aktual."""
    ...
    for idx, row in history.iterrows():
        ...
        with st.expander(label, expanded=False):
            ...
            with col2:
                ...
                prediction_ou = row.get('prediction_ou', 0)
                if prediction_ou == 1:
                    odds = row.get('current_over_odds', 0)
                else:
                    odds = row.get('current_under_odds', 0)
...
def render_history_manager():
    ...
    try:
        ...
        history = pd.DataFrame()
    if history.empty:
        st.info("History kosong."); return

    display_cols = [
        "league_code", "home_team", "away_team", "prediction",
        "prediction_status", "home_goals", "away_goals", "actual_over", "settlement_time"
    ]
    available_cols = [c for c in display_cols if c in history.columns]
    st.dataframe(history[available_cols], use_container_width=True)
...
def render_pending():
    ...
    for idx, row in df.iterrows():
        label = f"▶ {row['home_team']} vs {row['away_team']} ({row['prediction_status']})"
        with st.expander(label, expanded=False):
...
def render_pending_card(row, idx, pending_all, db):
    ...
    with col1:
        with st.container():
            ...
            ht_home = st.number_input("HT Home", min_value=0, step=1,
                                      value=int(row['home_ht_goals']) if pd.notna(row.get('home_ht_goals')) else 0,
                                      key=f"ht_home_{idx}", label_visibility="collapsed")
            st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        with st.container():
            st.markdown('<div class="ht-score-card"><div class="label">HT AWAY</div>', unsafe_allow_html=True)
            ht_away = st.number_input("HT Away", min_value=0, step=1,
                                      value=int(row['away_ht_goals']) if pd.notna(row.get('away_ht_goals')) else 0,
                                      key=f"ht_away_{idx}", label_visibility="collapsed")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col3:
        with st.container():
            ...
            ft_home = st.number_input("FT Home", min_value=0, step=1,
                                      value=int(row['home_goals']) if pd.notna(row.get('home_goals')) else 0,
                                      key=f"ft_home_{idx}", label_visibility="collapsed")
            st.markdown('</div>', unsafe_allow_html=True)
    with col4:
        with st.container():
            st.markdown('<div class="ft-score-card"><div class="label">FT AWAY</div>', unsafe_allow_html=True)
            ft_away = st.number_input("FT Away", min_value=0, step=1,
                                      value=int(row['away_goals']) if pd.notna(row.get('away_goals')) else 0,
                                      key=f"ft_away_{idx}", label_visibility="collapsed")
            st.markdown('</div>', unsafe_allow_html=True)
...
def save_pending_score(idx, ht_home, ht_away, ft_home, ft_away, pending_all, db):
    ...
    if original[PendingSchema.PREDICTION_STATUS] not in editable_status:
        st.error(f"Status `{original['prediction_status']}` tidak dapat diubah.")
        return
    ...
    if valid:
        ...
        # Update dataset_withgoal.csv menggunakan identitas 4 kolom
        match_id = (
            str(original.get("league_code", "")),
            str(original.get("kickoff_time", "")),
            str(original.get("home_team", "")),
            str(original.get("away_team", "")),
        )
        ...
        db.save_pending(pending_all)
        add_transaction(TransactionAction.VALIDATION, original['match_uid'],
                        "SCORE_ENTERED", f"Validation failed: {'; '.join(errors)}")
    ...
    def run_analysis(self, uploaded_file) -> dict:
        ...
        if context.get("validation_success",False):
            ...
            try:
                ...
                add(f"CHECKPOINT 2a: dataset write FAILED - {e}")
        else:
