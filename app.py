...
def normalize_correct_score_history(df: pd.DataFrame) -> pd.DataFrame:
    """Recalculate legacy Correct Score stakes/profit using the current rule.

    Only rows with valid CS odds (> 1) are recalculated. Rows without sufficient
    odds data retain their existing stake/profit values so historical data is not
    fabricated. The returned dataframe is a copy and does not persist changes.
    """
    ...
    for idx, row in out.iterrows():
        ...
        for i in range(1, 4):
            ...
            try:
                ...
                continue
            if not np.isfinite(odds) or odds <= 1.0 or pd.isna(score_raw):
                continue
...
def render_cs_history(session: SessionManager, storage: StorageProvider):
    ...
    # Recalculate legacy CS records for display and profit analysis.
