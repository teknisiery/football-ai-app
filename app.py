# ============================================================
# FOOTBALL AI V2 STABLE REV9 – DIAGNOSTIC INSTRUMENTATION
# Instrumentasi minimal untuk root cause investigation
# Semua log masuk ke st.session_state["debug_trace"] + print() ke server log
# ============================================================
...
# ============================================================
# GITHUB STORAGE PROVIDER (dengan instrumentasi diagnostik)
# ============================================================
class GitHubStorageProvider(StorageProvider):
    ...
    def load_dataframe(self, resource: Resource) -> pd.DataFrame:
        url = self._get_file_url(resource)
        trace = st.session_state.setdefault("debug_trace", [])
        # --- DIAGNOSTIC: hanya untuk resource PENDING ---
        if resource.id == "pending":
            try:
                r = requests.get(url, headers=self._get_headers())
                trace.append(f"[GH load_dataframe(pending)] HTTP Status: {r.status_code}")
                print(f"[DIAGNOSTIC] GitHubStorageProvider.load_dataframe(pending) | HTTP Status: {r.status_code}", flush=True)
                trace.append(f"[GH load_dataframe(pending)] Response Headers: {dict(r.headers)}")
                content_b64 = ""
                if r.status_code == 200:
                    content_b64 = r.json().get("content", "")
                    trace.append(f"[GH load_dataframe(pending)] Content Length (base64): {len(content_b64)}")
                    decoded = base64.b64decode(content_b64)
                    trace.append(f"[GH load_dataframe(pending)] Decoded Length: {len(decoded)}")
                    csv_preview = decoded[:300].decode('utf-8', errors='replace')
                    trace.append(f"[GH load_dataframe(pending)] Decoded CSV (first 300 chars):\n{csv_preview}")
                    try:
                        df = pd.read_csv(BytesIO(decoded))
                        trace.append(f"[GH load_dataframe(pending)] DataFrame Shape: {df.shape}")
                        trace.append(f"[GH load_dataframe(pending)] DataFrame Columns: {df.columns.tolist()}")
                        trace.append(f"[GH load_dataframe(pending)] DataFrame Head(3):\n{df.head(3).to_string()}")
                        print(f"[DIAGNOSTIC] GitHubStorageProvider.load_dataframe(pending) | Shape: {df.shape} | Columns: {df.columns.tolist()}", flush=True)
                    except Exception as e:
                        trace.append(f"[GH load_dataframe(pending)] Exception Type: {type(e).__name__}")
                        trace.append(f"[GH load_dataframe(pending)] Exception Message: {str(e)}")
                        trace.append(f"[GH load_dataframe(pending)] Full Traceback:\n{traceback.format_exc()}")
                elif r.status_code == 404:
                    trace.append("[GH load_dataframe(pending)] HTTP 404 – file not found (return empty DataFrame)")
                else:
                    trace.append(f"[GH load_dataframe(pending)] Unexpected status code: {r.status_code}")
            except Exception as e:
                trace.append(f"[GH load_dataframe(pending)] Exception during diagnostic: {type(e).__name__}: {e}")
                trace.append(f"[GH load_dataframe(pending)] Full Traceback:\n{traceback.format_exc()}")
...
# ============================================================
# DATABASE MANAGER (dengan instrumentasi di load_pending)
# ============================================================
class DatabaseManager:
    ...
    def load_pending(self) -> pd.DataFrame:
        ...
        trace = st.session_state.setdefault("debug_trace", [])
        trace.append(f"[DatabaseManager.load_pending] Shape: {df.shape}")
        trace.append(f"[DatabaseManager.load_pending] Columns: {df.columns.tolist()}")
        trace.append(f"[DatabaseManager.load_pending] Empty: {df.empty}")
        print(f"[DIAGNOSTIC] DatabaseManager.load_pending | Shape: {df.shape} | Columns: {df.columns.tolist()} | Empty: {df.empty}", flush=True)
        return df
...
# ============================================================
# SYNC ENGINE (dengan instrumentasi di sync_all_validated)
# ============================================================
class SyncEngine:
...
# ============================================================
# HISTORY SYNC ENGINE (dengan instrumentasi di sync_all_validated)
# ============================================================
class HistorySyncEngine:
    ...
    def sync_all_validated(self) -> int:
        ...
        trace = st.session_state.setdefault("debug_trace", [])
        trace.append(f"[SyncEngine.sync_all_validated] Pending Shape: {pending.shape}")
        trace.append(f"[SyncEngine.sync_all_validated] Pending Columns: {pending.columns.tolist()}")
        print(f"[DIAGNOSTIC] SyncEngine.sync_all_validated | Pending Shape: {pending.shape} | Columns: {pending.columns.tolist()}", flush=True)

        # --- Titik crash yang dilaporkan ---
        try:
            validated = pending[pending[PendingSchema.PREDICTION_STATUS] == MatchStatus.VALIDATED.value]
        except KeyError as e:
            trace.append(f"[SyncEngine.sync_all_validated] Exception Line: validated = pending[pending[PendingSchema.PREDICTION_STATUS] ...]")
            trace.append(f"[SyncEngine.sync_all_validated] Exception Type: {type(e).__name__}")
            trace.append(f"[SyncEngine.sync_all_validated] Exception Message: {str(e)}")
            trace.append(f"[SyncEngine.sync_all_validated] Full Traceback:\n{traceback.format_exc()}")
            print(f"[DIAGNOSTIC] SyncEngine.sync_all_validated | KeyError: {e}", flush=True)
            raise
        # --- End diagnostic ---
        ...
        for idx, row in validated.iterrows():
            ...
            pending.at[idx, 'settlement_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            synced += 1
...
def render_pending():
    ...
    # Pilih kolom yang aman untuk data_editor (hanya tipe sederhana)
    editor_columns = [
    ...
    column_config = {
        ...
        "ht_home_input": st.column_config.NumberColumn("HT Home", min_value=0, step=1, disabled=~df['is_editable']),
        "ht_away_input": st.column_config.NumberColumn("HT Away", min_value=0, step=1, disabled=~df['is_editable']),
        "ft_home_input": st.column_config.NumberColumn("FT Home", min_value=0, step=1, disabled=~df['is_editable']),
        "ft_away_input": st.column_config.NumberColumn("FT Away", min_value=0, step=1, disabled=~df['is_editable']),
    }
    ...
    if st.button("💾 Save Changes"):
        ...
        for idx, row in edited_df.iterrows():
            # idx adalah indeks asli dari pending_all
