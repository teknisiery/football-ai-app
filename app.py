...
def _valid_correct_score_leg(row: pd.Series, index: int):
    """Return parsed CS leg only when score and odds are both valid."""
    score_raw = row.get(f'cs_score_{index}')
    odds_raw = row.get(f'cs_odds_{index}')
    if pd.isna(score_raw) or pd.isna(odds_raw):
        return None
    try:
        score_h, score_a = map(int, str(score_raw).strip().split(':'))
        odds = float(odds_raw)
    except (TypeError, ValueError):
        return None
    if not np.isfinite(odds) or odds <= 1.0:
        return None
    return score_h, score_a, odds


def _has_complete_correct_score(row: pd.Series) -> bool:
    """A CS history bet is eligible only when CS1/CS2/CS3 and all odds exist."""
    return all(_valid_correct_score_leg(row, i) is not None for i in range(1, 4))


def normalize_correct_score_history(df: pd.DataFrame) -> pd.DataFrame:
    """Recalculate only complete legacy Correct Score records.

    A row is eligible only when all three recommendations (CS1/CS2/CS3) and
    their corresponding odds are present and valid. Incomplete rows are left
    untouched and are excluded from Correct Score profit statistics.
    """
    ...
    for idx, row in out.iterrows():
        if not _has_complete_correct_score(row):
            continue

        try:
            home_goals = int(float(row.get('home_goals')))
            away_goals = int(float(row.get('away_goals')))
        except (TypeError, ValueError):
            continue

        normalized_profit = 0.0
        for i in range(1, 4):
            parsed = _valid_correct_score_leg(row, i)
            if parsed is None:
                break
            score_h, score_a, odds = parsed
            stake = normalize_correct_score_stake(odds)
            if stake <= 0.0:
                break
            out.at[idx, f'cs_stake_{i}'] = stake
            if score_h == home_goals and score_a == away_goals:
                normalized_profit += stake * (odds - 1.0)
            else:
                normalized_profit -= stake
        else:
            out.at[idx, 'cs_profit'] = normalized_profit
...
def render_cs_history(session: SessionManager, storage: StorageProvider):
    ...
    # Recalculate only complete CS records for display and profit analysis.
    # A record is eligible only when CS1/CS2/CS3 and all three odds exist.
    # Stored history is not silently rewritten; persistence is explicit below.
    raw = normalize_correct_score_history(raw)
    complete_cs_mask = raw.apply(_has_complete_correct_score, axis=1)
    raw = raw.loc[complete_cs_mask].copy()
    if raw.empty:
        st.info("Belum ada data Correct Score lengkap (CS1, CS2, CS3 + odds) di history.")
        return
    ...
    raw['cs_profit'] = pd.to_numeric(raw['cs_profit'], errors='coerce')
    accumulated_profit_cs = raw['cs_profit'].dropna().sum()
    st.metric("Profit Akumulasi Correct Score", f"Rp {accumulated_profit_cs:+,.0f}")

    if st.button("💾 Simpan Normalisasi Correct Score ke History", key="save_cs_normalized_history"):
        db = DatabaseManager(storage)
        history = db.load_history()
        normalized_history = normalize_correct_score_history(history)
        db.save_history(normalized_history)
        st.success("History Correct Score berhasil dinormalisasi menggunakan stake terbaru.")
        st.rerun()
    ...
    # Hanya history tampilan 7 hari; akumulasi di atas tetap all-time.
    hari_ini = datetime.now().date()
    start_date = hari_ini - timedelta(days=6)
    df = df[(df['tanggal'] >= start_date) & (df['tanggal'] <= hari_ini)].copy()
    if df.empty:
    ...
    if df.empty:
        st.info("Data profit Correct Score belum tersedia untuk periode ini.")
        return
