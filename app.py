# ============================================================
# FOOTBALL AI V2 STABLE REV23 – Raw + Training Dataset Pipeline
# ============================================================
...
def append_raw_dataset(uploaded_df: pd.DataFrame):
    ...
    # Gunakan dedup untuk mengganti jika identity sudah ada
    updated = _append_dataset(existing, uploaded_df, dedup=True)
    db.save_dataset(updated)

def append_training_dataset(uploaded_df: pd.DataFrame):
    """Copy raw data, tambah kolom goal, simpan ke dataset_withgoal.csv."""
    df = uploaded_df.copy()
...
def update_dataset_goals(match_id: Tuple[str, str, str, str],
    ...
    """Update goal columns di dataset_withgoal.csv."""
    db = DatabaseManager()
    try:
...
class ApplicationController:
    ...
    def run_analysis(self, uploaded_file) -> dict:
        ...
        if context.get("validation_success",False):
            ...
            # Pipeline dataset baru
            try:
                upload_df = get_uploaded_dataframe()
                append_raw_dataset(upload_df)          # Raw dataset
                append_training_dataset(upload_df)     # Training dataset
