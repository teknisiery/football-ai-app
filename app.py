...
# ============================================================
# EXPECTED FEATURES LIST (53 fitur)
# ============================================================
EXPECTED_FEATURES = [
...
def append_raw_dataset(uploaded_df: pd.DataFrame, home_team: str, away_team: str):
    ...
    # Hitung ulang over_move, under_move, ou_movement dari odds
    if 'open_over_odds' in df.columns and 'current_over_odds' in df.columns:
        df['over_move'] = df['current_over_odds'] - df['open_over_odds']
    if 'open_under_odds' in df.columns and 'current_under_odds' in df.columns:
        df['under_move'] = df['current_under_odds'] - df['open_under_odds']
    if 'over_move' in df.columns and 'under_move' in df.columns:
...
def append_training_dataset(uploaded_df: pd.DataFrame, home_team: str, away_team: str):
    ...
    for col in ["home_ht_goals","away_ht_goals","home_goals","away_goals"]:
        df[col] = np.nan
    # Tambahkan kolom totalgol_ft dan totalgol_ht
    df["totalgol_ft"] = np.nan
    df["totalgol_ht"] = np.nan
    df = _reorder_columns(df, extra_cols=["home_ht_goals","away_ht_goals","home_goals","away_goals","totalgol_ft","totalgol_ht"])
    db = DatabaseManager()
...
def update_dataset_goals(match_id: Tuple[str, str, str, str],
    ...
    try:
        ...
        if match_condition.any():
            ...
            # Hitung total gol
            dataset.at[idx, "totalgol_ft"] = home_ft + away_ft
            dataset.at[idx, "totalgol_ht"] = home_ht + away_ht
...
def update_league_profile(league_code):
    db = DatabaseManager()
    try:
        df = db.load_dataset_with_goal()
    except:
        return
    # Pastikan kolom totalgol_ft dan totalgol_ht ada, jika tidak hitung
    ...
    if 'totalgol_ht' not in df.columns:
        if 'home_ht_goals' in df.columns and 'away_ht_goals' in df.columns:
            df['totalgol_ht'] = df['home_ht_goals'] + df['away_ht_goals']
        else:
            df['totalgol_ht'] = np.nan
    ...
    total = len(df_league)
    avg_goals = df_league['totalgol_ft'].mean()
    over25_pct = (df_league['totalgol_ft'] > 2.5).mean()
    btts_pct = df_league['btts'].mean()
    under35_pct = (df_league['totalgol_ft'] < 3.5).mean()
    eg_p25 = df_league['totalgol_ft'].quantile(0.25)
    eg_p75 = df_league['totalgol_ft'].quantile(0.75)
    # Baca profil yang sudah ada
    try:
        profile_df = db.load_league_profile()
    except:
    ...
    new_row = {
        ...
        'ht0_p25': 0.0,
        'ht0_p75': 0.0,
        'ev_over_threshold': 0.01,
        'ev_under_threshold': 0.02,
        'total_matches': total
    }
    if profile_df.empty or league_code not in profile_df['league_code'].values:
        profile_df = pd.concat([profile_df, pd.DataFrame([new_row])], ignore_index=True)
    else:
        idx = profile_df[profile_df['league_code'] == league_code].index[0]
        for key, value in new_row.items():
            profile_df.at[idx, key] = value
    db.save_league_profile(profile_df)
...
def build_pending(ctx: PredictionContext) -> PendingResult:
    ...
    try:
        ...
        if not history.empty and PendingSchema.MATCH_UID in history.columns:
            if identity[PendingSchema.MATCH_UID] in history[PendingSchema.MATCH_UID].values:
                return PendingResult(
                    False, 
                    "❌ Pertandingan ini sudah ada di History dan tidak dapat disimpan lagi.",
                    home_team=ctx.home_team,
                    away_team=ctx.away_team,
                    match_uid=identity[PendingSchema.MATCH_UID]
                )
    except Exception:
        pass  # Jika gagal membaca history, lanjutkan saja
...
def _settlement_result(row):
    ...
    # Coba ambil prediction_ou, jika tidak ada atau NaN, coba dari kolom prediction
    if 'prediction_ou' in row and pd.notna(row.get('prediction_ou')):
        pred_ou = int(row['prediction_ou'])
    else:
        # Ambil dari string prediksi, misal "OVER 2.5" → True, "UNDER 2.75" → False
...
def render_settlement():
    ...
    db = DatabaseManager()
    try:
        raw_df = db.storage.load_dataframe(ResourceRegistry.HISTORY)
    except Exception as e:
        st.error(f"Gagal membaca history: {e}")
        return
    
    if raw_df.empty:
        st.info("Belum ada data di History.")
        return
    
    # Urutkan dan ambil 20 terbaru
    if 'settlement_time' in raw_df.columns:
        raw_df = raw_df.sort_values('settlement_time', ascending=False)
    elif 'kickoff_time' in raw_df.columns:
        raw_df = raw_df.sort_values('kickoff_time', ascending=False)
    raw_df = raw_df.head(20)
    
    # Total profit 20 terbaru
    if 'profit' in raw_df.columns:
...
def render_history_manager():
    ...
    # Coba baca langsung tanpa normalisasi
    try:
        raw_df = db.storage.load_dataframe(ResourceRegistry.HISTORY)
    except Exception as e:
        st.error(f"Gagal membaca history: {e}")
    ...
    st.caption(f"🔍 Raw data: {len(raw_df)} baris")
    
    # Jika raw_df tidak kosong, tampilkan langsung (fallback)
    if not raw_df.empty:
        # Urutkan dan ambil 20 terbaru
        if 'settlement_time' in raw_df.columns:
            raw_df = raw_df.sort_values('settlement_time', ascending=False)
        ...
        display_cols = [
            "settlement_time", "home_team", "away_team", "prediction",
            "home_goals", "away_goals", "profit", "result"
        ]
        available = [c for c in display_cols if c in raw_df.columns]
        st.dataframe(raw_df[available], use_container_width=True)
        return
...
def render_learning():
    ...
    if not db.is_history_ready():
        st.warning("History kosong. Tidak bisa melatih model.")
        return
    
    st.write(f"Jumlah data di History: {len(db.load_history())}")
    
    if st.button("🚀 Latih Ulang Model dari History", type="primary"):
        with st.spinner("Melatih model baru... Mohon tunggu sebentar."):
            try:
                history = db.load_history()
                if history.empty:
                    st.error("History kosong.")
                    return
                
                # Jika totalgol_ft tidak ada, hitung dari home_goals + away_goals
                if 'totalgol_ft' not in history.columns:
                    if 'home_goals' in history.columns and 'away_goals' in history.columns:
                        history['totalgol_ft'] = history['home_goals'] + history['away_goals']
                    else:
                        st.error("Data history tidak memiliki kolom gol.")
                        return
                
                # Hanya gunakan fitur yang tersedia
                feature_cols = [c for c in EXPECTED_FEATURES if c in history.columns]
                if len(feature_cols) < 40:
                    st.error(f"Kolom fitur tidak cukup di history. Hanya {len(feature_cols)} fitur.")
                    return
                
                X = history[feature_cols].fillna(0)
                y = history['totalgol_ft']
                ...
                db.save_model({'model': model, 'feature_cols': feature_cols})
                st.session_state.model = model
                st.session_state.feature_cols = feature_cols
                
                st.success(f"✅ Model berhasil dilatih ulang dari {len(history)} data!")
                st.balloons()
            except Exception as e:
...
def save_pending_score(idx, ht_home, ht_away, ft_home, ft_away, pending_all, db):
    ...
    if valid:
        ...
        # Tambahkan totalgol_ft ke record jika belum ada
        if 'totalgol_ft' not in record:
            record['totalgol_ft'] = ft_home + ft_away
        if 'totalgol_ht' not in record:
            record['totalgol_ht'] = ht_home + ht_away
        history = pd.concat([history, pd.DataFrame([record])], ignore_index=True)
        db.save_history(history)

        # Perbarui profil liga jika satu ronde selesai
        league = int(original.get('league_code', 0))
        config = LEAGUE_ROUND_CONFIG.get(league)
        if config:
            try:
                df_withgoal = db.load_dataset_with_goal()
                df_league = df_withgoal[df_withgoal['league_code'] == league]
                matches_with_score = len(df_league.dropna(subset=['totalgol_ft']))
            except:
                matches_with_score = 0
            if matches_with_score % config['matches_per_round'] == 0:
                update_league_profile(league)
...
def render_sidebar():
    with st.sidebar:
        ...
        # Tombol manual perbarui semua profil liga
        if st.button("🔄 Perbarui Semua Profil Liga"):
            for lg in LEAGUE_ROUND_CONFIG.keys():
                update_league_profile(lg)
            st.success("Semua profil liga diperbarui!")
            st.rerun()
...
def main():
    ...
    if token:
        # Ambil repo full dari secrets (format "owner/repo")
        repo_full = (
            os.getenv("GITHUB_REPOSITORY") or 
            st.secrets.get("GITHUB_REPOSITORY") or 
            os.getenv("GITHUB_REPO") or 
            st.secrets.get("GITHUB_REPO") or 
            "teknisiery/football-ai-db"
        )
        if "/" in repo_full:
            parts = repo_full.split("/")
            owner, repo = parts[0], parts[1]
        else:
            owner = os.getenv("GITHUB_USERNAME") or st.secrets.get("GITHUB_USERNAME") or "teknisiery"
            repo = repo_full

        branch = os.getenv("GITHUB_BRANCH") or st.secrets.get("GITHUB_BRANCH") or "main"
