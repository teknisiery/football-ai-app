...
# + Perbaikan evaluate_model: cache manual
# ============================================================
...
# ============================================================
# MODEL EVALUATION (CACHE MANUAL)
# ============================================================
def evaluate_model(storage: StorageProvider) -> dict:
    """Hitung MAE dan RMSE dari history yang sudah lengkap. Cache manual."""
    ...
    try:
        ...
        result = {"mae": None, "rmse": None, "message": f"Gagal membaca history: {e}"}
        st.session_state[cache_key] = result
        return result

    if df.empty:
        result = {"mae": None, "rmse": None, "message": "History kosong."}
        st.session_state[cache_key] = result
        return result
    ...
    if 'totalgol_ft' not in df.columns:
        if 'home_goals' in df.columns and 'away_goals' in df.columns:
            ...
            result = {"mae": None, "rmse": None, "message": "Data tidak memiliki kolom skor (home_goals/away_goals)."}
            st.session_state[cache_key] = result
            return result

    if 'expected_goal' not in df.columns:
        result = {"mae": None, "rmse": None, "message": "Data tidak memiliki kolom expected_goal."}
        st.session_state[cache_key] = result
        return result

    valid = df.dropna(subset=['expected_goal', 'totalgol_ft'])
    if len(valid) < 2:
        result = {"mae": None, "rmse": None, "message": "Belum cukup data untuk evaluasi (minimal 2 data)."}
        st.session_state[cache_key] = result
        return result
    ...
    result = {"mae": mae, "rmse": rmse, "message": None}
    st.session_state[cache_key] = result
    return result
...
def render_learning(session: SessionManager, storage: StorageProvider):
    ...
    if st.button("🚀 Latih Ulang Model"):
        with st.spinner("Melatih..."):
            ...
            st.cache_data.clear()   # bersihkan cache evaluasi & lainnya
