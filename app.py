...
class PendingContract:
    CORE_COLUMNS = [
        ...
        "prediction_1x2_raw", "stake_1x2_raw", "one_x_two_filter_reason", "one_x_two_filtered",
        # BTTS
...
def render_prediction_card(summary: dict):
    ...
    if rec_btts:
        ...
        if rec_btts != 'NO BET' and stake_btts > 0:
            ...
            yes_o = summary.get('market_odds_btts_yes')
            no_o = summary.get('market_odds_btts_no')
            odds_bits = []
            if yes_o is not None: odds_bits.append(f"Y {float(yes_o):.2f}")
            if no_o is not None: odds_bits.append(f"N {float(no_o):.2f}")
            if odds_bits:
                btts_value += f"<br><small>Odds: {' / '.join(odds_bits)} | Rp0</small>"
    else:
    ...
    one_x_two_card_value = rec_1x2 if rec_1x2 else "N/A"
    if rec_1x2 == 'NO BET':
        raw_1x2 = summary.get('prediction_1x2_raw')
        raw_odds = summary.get('market_odds_1x2', {}).get(str(raw_1x2).lower()) if raw_1x2 else None
        if raw_odds is not None:
            one_x_two_card_value += f"<br><small>{raw_1x2} @ {float(raw_odds):.2f}</small>"

    cards_row2 = [
        ("🎯", "1X2",
         one_x_two_card_value,
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    prediction_1x2 = None
    stake_1x2 = 0.0
    prediction_1x2_raw = None
    stake_1x2_raw = 0.0
    one_x_two_filter_reason = None
    one_x_two_filtered = False
            ...
            for outcome in ['home', 'draw', 'away']:
                ...
                if prob and odds and odds > 0:
                    ...
                    if outcome == 'home':
                        ...
                        ev_1x2_away = ev
            all_ev_list = [('HOME', ev_1x2_home), ('DRAW', ev_1x2_draw), ('AWAY', ev_1x2_away)]
            all_ev_list = [(label, ev) for label, ev in all_ev_list if ev is not None]
            if all_ev_list:
                prediction_1x2_raw, best_raw_ev = max(all_ev_list, key=lambda x: x[1])
                raw_odds = odds_1x2_dict.get(prediction_1x2_raw.lower(), 0)
                stake_1x2_raw = 100000.0 / (raw_odds - 1.0) if raw_odds and raw_odds > 1.0 else 0.0
            else:
                prediction_1x2_raw = None
                stake_1x2_raw = 0.0

            ev_list = [(label, ev) for label, ev in all_ev_list if ev > ev_th_over]
            if ev_list:
                candidate, best_ev = max(ev_list, key=lambda x: x[1])
                outcome_key = candidate.lower()
                market_odds = odds_1x2_dict.get(outcome_key, 0)
                prediction_1x2, stake_1x2 = apply_1x2_odds_floor(
                    candidate, market_odds, target_net_profit=100000.0
                )
                if prediction_1x2 == 'NO BET':
                    one_x_two_filtered = True
                    one_x_two_filter_reason = (
                        f"ODDS_BELOW_MIN_{ONE_X_TWO_MIN_ODDS:.2f}"
                        if market_odds < ONE_X_TWO_MIN_ODDS
                        else "INVALID_ODDS"
                    )
                else:
                    one_x_two_filter_reason = "ELIGIBLE"
            else:
                prediction_1x2 = 'NO BET'
                stake_1x2 = 0.0
                one_x_two_filtered = True
                one_x_two_filter_reason = "EV_BELOW_THRESHOLD"
    else:
        prob_1x2_hybrid_final = prob_1x2_hybrid
        prediction_1x2 = 'NO BET'
        stake_1x2 = 0.0
        one_x_two_filter_reason = "NO_MARKET_ODDS"
    ...
    r['prediction_1x2_raw'] = prediction_1x2_raw
    r['stake_1x2_raw'] = stake_1x2_raw
    ...
    return {
        ...
        "stake_1x2": stake_1x2,
        "prediction_1x2_raw": prediction_1x2_raw,
        "stake_1x2_raw": stake_1x2_raw,
        "one_x_two_filter_reason": one_x_two_filter_reason,
        "one_x_two_filtered": one_x_two_filtered,
        "movement": movement,
        # BTTS
    ...
    for idx, row in df.iterrows():
        ...
        with cols[1]:
            with st.expander(f"▶ {home_safe} vs {away_safe}"):
                ...
                # 1X2 — final decision controls stake; raw prediction/odds remain visible.
                pred_1x2 = str(row.get('prediction_1x2', '') or '').strip().upper()
                raw_1x2 = str(row.get('prediction_1x2_raw', '') or '').strip().upper()
                stake_1x2 = float(row.get('stake_1x2', 0) or 0)
                display_1x2 = raw_1x2 or (pred_1x2 if pred_1x2 != 'NO BET' else '')
                odds_1x2 = row.get(f"market_odds_1x2_{display_1x2.lower()}") if display_1x2 in {'HOME','DRAW','AWAY'} else None
                if pred_1x2 in {'HOME','DRAW','AWAY'} and stake_1x2 > 0:
                    odds_text = f" @{float(odds_1x2):.2f}" if odds_1x2 is not None and not pd.isna(odds_1x2) else ''
                    lines.append(f"1X2 | {pred_1x2}{odds_text} | Rp{stake_1x2:,.0f}")
                elif pred_1x2 == 'NO BET':
                    odds_text = f" @{float(odds_1x2):.2f}" if odds_1x2 is not None and not pd.isna(odds_1x2) else ''
                    reason = row.get('one_x_two_filter_reason') or 'NO_STAKE'
                    raw_text = f" {raw_1x2}" if raw_1x2 else ''
                    lines.append(f"1X2 | NO BET{raw_text}{odds_text} | Rp0 | {reason}")

                # BTTS — keep both market prices visible even when final decision is NO BET.
                rec_btts = str(row.get('recommendation_btts', '') or '').strip().upper()
                stake_btts = float(row.get('stake_btts', 0) or 0)
                if rec_btts in {'YES','NO'} and stake_btts > 0:
                    odds_col = 'market_odds_btts_yes' if rec_btts == 'YES' else 'market_odds_btts_no'
                    odds_btts = row.get(odds_col)
                    odds_text = f" @{float(odds_btts):.2f}" if odds_btts is not None and not pd.isna(odds_btts) else ''
                    lines.append(f"BTTS | {rec_btts}{odds_text} | Rp{stake_btts:,.0f}")
                elif rec_btts == 'NO BET':
                    yes_o = row.get('market_odds_btts_yes')
                    no_o = row.get('market_odds_btts_no')
                    odds_parts = []
                    if yes_o is not None and not pd.isna(yes_o): odds_parts.append(f"YES @{float(yes_o):.2f}")
                    if no_o is not None and not pd.isna(no_o): odds_parts.append(f"NO @{float(no_o):.2f}")
                    reason = row.get('btts_filter_reason') or 'NO_STAKE'
                    odds_text = ' / '.join(odds_parts)
                    lines.append(f"BTTS | NO BET | {odds_text} | Rp0 | {reason}")
...
def render_match_pnl(session: SessionManager, storage: StorageProvider):
    """Render compact per-match P&L grouped by recent settlement date."""
    ...
    df = raw.copy()
    time_col = "settlement_time" if "settlement_time" in df.columns else None
    if time_col:
        df["_match_pnl_dt"] = pd.to_datetime(df[time_col], errors="coerce")
        df = df.sort_values("_match_pnl_dt", ascending=False, na_position="last")
    else:
        df["_match_pnl_dt"] = pd.NaT

    df = df.drop_duplicates(subset=["match_uid"], keep="first")

    # Build deterministic ledgers once. Rendering is intentionally compact:
    # one colored row per match, with details available only when selected.
    results = []
    for _, row in df.iterrows():
        try:
            result = build_match_pnl(row.to_dict())
            result["_date"] = row.get("_match_pnl_dt")
    ...
    total_stake = sum(float(r.get("total_stake", 0) or 0) for r in results)
    total_return = sum(float(r.get("total_return", 0) or 0) for r in results)
    total_net = sum(float(r.get("net_pnl", 0) or 0) for r in results)
    st.caption(f"{len(results)} pertandingan dengan minimal satu taruhan. Setiap pertandingan dihitung sebagai satu portfolio P&L.")
    st.markdown(
        f"**TOTAL MODAL** Rp{total_stake:,.0f} &nbsp;&nbsp; "
        f"**TOTAL RETURN** Rp{total_return:,.0f} &nbsp;&nbsp; "
        f"**NET P/L** <span style='color:{'#16a34a' if total_net > 0 else '#ef4444' if total_net < 0 else '#94a3b8'}; font-weight:800;'>Rp{total_net:+,.0f}</span>",
        unsafe_allow_html=True,
    )
    ...
    for result in results:
        dt = result.get("_date")
        d = dt.date() if pd.notna(dt) else None
        if d is not None and 0 <= (today - d).days <= 6:
            grouped.setdefault(d, []).append(result)
        else:
            older.append(result)
    ...
    # Only the requested recent seven calendar days are expanded groups.
    # Match rows themselves are compact, so the browser is not flooded with
    # nested cards/tables.
    for offset in range(7):
        ...
        if not group:
            continue
        day_net = sum(float(x.get("net_pnl", 0) or 0) for x in group)
        sign = "+" if day_net > 0 else ""
        with st.expander(f"▼ {day_label(d)} — Rp {sign}{day_net:,.0f}", expanded=(offset == 0)):
            group = sorted(group, key=lambda x: x.get("_date") if pd.notna(x.get("_date")) else pd.Timestamp.min, reverse=True)
            for result in group:
                home = safe_html(str(result.get("home_team", "")))
                away = safe_html(str(result.get("away_team", "")))
                score = f"{int(result.get('home_goals', 0))}-{int(result.get('away_goals', 0))}"
                net = float(result.get("net_pnl", 0) or 0)
                if net > 0:
                    bg = "#16a34a"
                elif net < 0:
                    bg = "#ef4444"
                    ...
                    bg = "#475569"
                st.markdown(
                    f"<div style='background:{bg}; color:white; padding:9px 12px; "
                    f"border-radius:9px; margin:4px 0; font-weight:700;'>"
                    f"⚽ {home} vs {away} | {score} | Rp{net:+,.0f}</div>",
                    unsafe_allow_html=True,
                )

                # The detail is rendered as a compact single-header table only
                # when the user explicitly selects the match via a checkbox.
                detail_key = f"pnl_detail_{result.get('match_uid')}"
                if st.checkbox("Detail", key=detail_key, label_visibility="collapsed"):
                    st.markdown(f"**Total Modal** Rp{float(result.get('total_stake', 0) or 0):,.0f}", unsafe_allow_html=True)
                    if result.get("legs"):
                        st.markdown(format_pnl_table_html(result["legs"]), unsafe_allow_html=True)
                    st.markdown(
                        f"**TOTAL MODAL** Rp{float(result.get('total_stake', 0) or 0):,.0f} &nbsp; | &nbsp; "
                        f"**TOTAL RETURN** Rp{float(result.get('total_return', 0) or 0):,.0f} &nbsp; | &nbsp; "
                        f"**NET P/L** Rp{net:+,.0f}"
                    )

    if older:
        older_net = sum(float(x.get("net_pnl", 0) or 0) for x in older)
        st.caption(f"Riwayat lebih lama: {len(older)} pertandingan | Net P/L Rp{older_net:+,.0f}. Data tetap tersimpan untuk audit.")
        ...
        if session.get_prediction_state().processed:
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ...
                    row['prediction_1x2_raw'] = summary.get('prediction_1x2_raw')
                    row['stake_1x2_raw'] = summary.get('stake_1x2_raw', 0)
                    row['one_x_two_filter_reason'] = summary.get('one_x_two_filter_reason')
                    row['one_x_two_filtered'] = summary.get('one_x_two_filtered', False)
