...
OPTIONAL_RESOURCES = {
    ...
    ResourceRegistry.LEAGUE_PROFILE.id,  # tambahan perbaikan
}
...
# ============================================================
# LEAGUE PROFILE (dengan caching)
# ============================================================
@st.cache_data(ttl=600)
def get_league_profile(storage: StorageProvider) -> pd.DataFrame:
    """Ambil profil liga dari storage, gunakan caching."""
    if storage.exists(ResourceRegistry.LEAGUE_PROFILE):
        return storage.load_dataframe(ResourceRegistry.LEAGUE_PROFILE)
    return pd.DataFrame()
...
def update_league_profile(storage: StorageProvider, league_code: int):
    ...
    get_league_profile.clear()  # bersihkan cache setelah update
...
def render_learning(session: SessionManager, storage: StorageProvider):
    ...
    if st.button("🚀 Latih Ulang Model"):
        with st.spinner("Melatih..."):
            ...
            model.fit(X, y)
            # Simpan ke app_storage (lokal) dan hapus cache
...
def render_debug(session: SessionManager):
    ...
    if not trace: st.info("No trace")
        ...
        for line in trace[-100:]: st.text(line)  # hanya tampilkan 100 terakhir
