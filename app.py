...
# Correct Score profit ledger reset.
# Existing CS history is preserved for archive/model evaluation, but only
# records explicitly marked active after this reset participate in CS profit.
CS_PROFIT_RESET_DATE = datetime(2026, 8, 9).date()
CS_PROFIT_ACTIVE_COLUMN = "cs_profit_active"
...
def load_css():
    ...
    st.markdown("""
    <style>
    /* Betting-history metrics: intentionally ~50% of Streamlit's large metric value. */
    .history-metric { margin: 10px 0 16px 0; }
    .history-metric-label {
        color: #a0a0b0; font-size: 0.72rem; line-height: 1.2;
        margin-bottom: 3px;
    }
    .history-metric-value {
        color: #f5f5f5; font-size: 1.25rem; line-height: 1.15;
        font-weight: 500;
    }
    </style>
...
def _has_complete_correct_score(row) -> bool:
    """Return True only when CS1/CS2/CS3 and all three odds are present."""
    for i in range(1, 4):
        score = row.get(f'cs_score_{i}')
        odds = row.get(f'cs_odds_{i}')
        if score is None or pd.isna(score) or str(score).strip() == '':
            return False
        try:
            if float(odds) <= 1.0:
                return False
...
def ensure_cs_profit_reset(storage: StorageProvider):
    """One-time migration: preserve old CS history but exclude it from new CS profit."""
    try:
    ...
    changed = False
    if CS_PROFIT_ACTIVE_COLUMN not in hist.columns:
        hist[CS_PROFIT_ACTIVE_COLUMN] = False
        changed = True
    else:
        missing = hist[CS_PROFIT_ACTIVE_COLUMN].isna()
        if missing.any():
            hist.loc[missing, CS_PROFIT_ACTIVE_COLUMN] = False
            changed = True

    # Only complete CS records can ever participate in the new profit ledger.
    if changed:
        storage.save_dataframe(ResourceRegistry.HISTORY, hist)


def render_history_metric(label: str, value: str):
    """Compact metric used only inside betting-history tabs."""
    st.markdown(
        f"<div class='history-metric'><div class='history-metric-label'>{safe_html(label)}</div>"
        f"<div class='history-metric-value'>{safe_html(value)}</div></div>",
        unsafe_allow_html=True
    )
            ...
            with st.expander(f"▶ {home_safe} vs {away_safe}"):
                ...
                if st.session_state.get(f"processing_{match_uid}", False):
                    ...
                    if valid:
                        ...
                        settlement_date = datetime.strptime(full_record['settlement_time'], "%Y-%m-%d %H:%M:%S").date()
                        full_record[CS_PROFIT_ACTIVE_COLUMN] = (
                            settlement_date >= CS_PROFIT_RESET_DATE
                            and _has_complete_correct_score(full_record)
                        )
...
def render_settlement(session: SessionManager, storage: StorageProvider):
    ...
    render_history_metric("Profit Akumulasi OU", f"Rp {accumulated_profit_ou:+,.0f}")
    ...
    render_history_metric("Total Profit Hari Ini", f"Rp {total_profit_hari_ini:+,.0f}")
...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    render_history_metric("Profit Akumulasi 1X2", f"Rp {accumulated_profit_1x2:+,.0f}")
    if df.empty:
    ...
    render_history_metric("Total Profit 1X2 (7 Hari)", f"Rp {total_profit:+,.0f}")
    if total_bets > 0:
        win_rate = win_count / total_bets * 100
        roi = total_profit / total_stake * 100 if total_stake > 0 else 0.0
        col1, col2, col3 = st.columns(3)
        render_history_metric("Win Rate", f"{win_rate:.1f}%")
        render_history_metric("ROI", f"{roi:.1f}%")
        render_history_metric("Akurasi", f"{win_count}/{total_bets}")
...
def render_btts_history(session: SessionManager, storage: StorageProvider):
    ...
    start_date = hari_ini - timedelta(days=6)
    df = df[(df['tanggal'] >= start_date) & (df['tanggal'] <= hari_ini)].copy()
    render_history_metric("Profit Akumulasi BTTS", f"Rp {accumulated_profit_btts:+,.0f}")
    if df.empty:
    ...
    render_history_metric("Total Profit BTTS (7 Hari)", f"Rp {total_profit:+,.0f}")
    if total_bets > 0:
        win_rate = win_count / total_bets * 100
        roi = total_profit / total_stake * 100 if total_stake > 0 else 0.0
        col1, col2, col3 = st.columns(3)
...
def render_cs_history(session: SessionManager, storage: StorageProvider):
    ...
    cs_cols = ['cs_score_1','cs_score_2','cs_score_3','cs_profit', CS_PROFIT_ACTIVE_COLUMN]
    ...
    # Correct Score profit was reset on 9 August 2026. Existing CS history
    # remains in HISTORY for archive/model evaluation but is excluded from
    # the new CS profit ledger. New settlements are marked active explicitly.
    if CS_PROFIT_ACTIVE_COLUMN not in raw.columns:
        st.info("Profit Correct Score baru dimulai 9 Agustus 2026. Belum ada data baru.")
        return

    df[CS_PROFIT_ACTIVE_COLUMN] = df[CS_PROFIT_ACTIVE_COLUMN].fillna(False).astype(bool)
    df = df[df[CS_PROFIT_ACTIVE_COLUMN]].copy()
    df['cs_profit'] = pd.to_numeric(df['cs_profit'], errors='coerce')
    df = df.dropna(subset=['cs_profit']).copy()
    if df.empty:
        render_history_metric("Profit Akumulasi Correct Score", "Rp +0")
        st.info("Belum ada data Correct Score baru sejak 9 Agustus 2026.")
        return

    accumulated_profit_cs = df['cs_profit'].sum()
    render_history_metric("Profit Akumulasi Correct Score", f"Rp {accumulated_profit_cs:+,.0f}")
    ...
    render_history_metric("Total Profit Correct Score (7 Hari)", f"Rp {total_cs_profit:+,.0f}")
    ...
    render_history_metric("Win Rate (salah satu dari 3 tepat)", f"{win_rate:.1f}%")

    display_cols = [c for c in available_cols if c in df.columns and c != CS_PROFIT_ACTIVE_COLUMN]
...
def main():
    ...
    # One-time CS profit reset: old history is retained but excluded from the
    # new Correct Score profit accumulation. New settlements are flagged active.
    ensure_cs_profit_reset(db_storage)
    ...
    if session.uploaded_file is None:
        ...
        tab1, tab2, tab2_5, tab2_6, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
            "📋 Pending",
            "📝 Settlement",
            "🎯 1X2",
            "🤝 BTTS",
