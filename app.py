...
def render_1x2_history(session: SessionManager, storage: StorageProvider):
    ...
    # Pisahkan taruhan aktif dan NO BET
    bets_df = df[df['prediction_1x2'] != 'NO BET'].copy()
    nobet_df = df[df['prediction_1x2'] == 'NO BET'].copy()

    if bets_df.empty and nobet_df.empty:
        st.info("Belum ada data 1X2.")
        return

    # ===== BAGIAN TARUHAN AKTIF =====
    if not bets_df.empty:
        ...
        if bets_df.empty:
            ...
            hari_ini = datetime.now().date()
            start_date = hari_ini - timedelta(days=6)
            bets_df = bets_df[(bets_df['tanggal'] >= start_date) & (bets_df['tanggal'] <= hari_ini)].copy()
            st.metric("Profit Akumulasi 1X2", f"Rp {accumulated_profit_1x2:+,.0f}")

            if bets_df.empty:
                st.info("Tidak ada data 1X2 aktif dalam 7 hari terakhir.")
            else:
                bets_df = bets_df.sort_values('settlement_dt', ascending=False)
                total_profit = bets_df['profit_1x2'].sum()
                total_bets = len(bets_df)
                win_count = (bets_df['result_1x2'].astype(str).str.upper() == 'WIN').sum()
                total_stake = pd.to_numeric(bets_df.get('stake_1x2', 0), errors='coerce').fillna(0).sum()
                ...
                grouped = bets_df.groupby('tanggal')
                tanggal_list = sorted(grouped.groups.keys(), reverse=True)
                hari_map = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu']
                bulan_map = {
    ...
    # ===== BAGIAN NO BET =====
    if not nobet_df.empty:
        st.markdown("### 🚫 1X2 NO BET")
        st.caption("Prediksi yang tidak menghasilkan taruhan (stake 0). Tetap tersimpan untuk observasi.")

        nobet_df['settlement_dt'] = pd.to_datetime(nobet_df.get('settlement_time'), errors='coerce')
        nobet_df = nobet_df.dropna(subset=['settlement_dt'])
        nobet_df_sorted = nobet_df.sort_values('settlement_dt', ascending=False)

        for _, row in nobet_df_sorted.iterrows():
            home = safe_html(str(row.get('home_team','')))
            away = safe_html(str(row.get('away_team','')))
            league = safe_html(str(row.get('league_name', '')))
            pred = row.get('prediction_1x2')
            ...
            st.markdown(
                f"<div style='background-color:#334155; padding:8px; border-radius:8px; "
                f"margin:4px 0; color:white;'>"
                f"<strong>{home} vs {away}</strong> | {league} | Pred: {pred} | "
                f"Skor: {score} | NO BET"
                f"</div>", unsafe_allow_html=True
...
def render_btts_history(session: SessionManager, storage: StorageProvider):
    ...
    if not bets_df.empty:
        ...
        if bets_df.empty:
            ...
            if bets_df.empty:
                ...
                total_profit = bets_df['profit_btts'].sum()
                total_bets = len(bets_df)
                win_count = (bets_df['result_btts'].astype(str).str.upper() == 'WIN').sum()
                total_stake = pd.to_numeric(bets_df.get('stake_btts', 0), errors='coerce').fillna(0).sum()
    ...
    # ===== BAGIAN NO BET =====
    if not nobet_df.empty:
        st.markdown("### 🚫 BTTS NO BET")
        st.caption("Prediksi BTTS yang tidak menghasilkan taruhan (stake 0).")
