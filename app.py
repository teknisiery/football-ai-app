...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    if score_probs:
        # Siapkan data untuk coherence filter
        ...
        # Konversi odds CS mentah menjadi probabilitas
        odds_dict_cs = session.uploaded_odds  # dictionary odds mentah
        fair_probs_cs = calculate_fair_probs(odds_dict_cs) if odds_dict_cs else None

        # Konversi odds 1X2 menjadi implied probabilities
        if odds_1x2_dict:
        ...
        if not passed:
            # Overwrite semua rekomendasi menjadi NO BET
            rec = "NO BET"
            rec_color = "d"
            stake = 0.0
