...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    # Legacy rows hanya dinormalisasi bila belum memiliki metadata filter.
    # Jangan save ke GitHub pada setiap rerun halaman; itu sangat mahal dan tidak perlu.
...
def render_match_pnl(session: SessionManager, storage: StorageProvider):
    """Compact 7-day MATCH P&L. Heavy detail is rendered only for the selected match."""
    ...
    # Build only the lightweight summary first. Full legs are calculated only
    # after the user clicks a match, which keeps the initial page fast.
    summaries = []
    ...
    for day_key, day_results in groups.items():
        ...
        # Only the current day is opened initially. Other days are lazy and do not
        # render their match rows until the user opens that day.
    ...
    # Detail is a single compact table: Jenis | Modal | Odds | Hasil | P/L.
...
def main():
    ...
    if session.uploaded_file is None:
        ...
        if session.get_prediction_state().processed:
            ...
            if combined_file:
                try:
                    ...
                    if combined_changed:
                        combined = parse_combined_odds_csv(combined_bytes)
                    ...
                    if combined_changed and combined.get('cs'):
                        ps = session.get_prediction_state()
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ...
                    # Serialize the payload directly. Do not reconstruct PredictionResult here:
                    # older Streamlit Session State may contain a payload from a previous app version.
        ...
        # Streamlit 1.55+ supports lazy tabs. We keep the original tab UI but
        # render only the active tab, preventing every history/P&L page from running
        # during an odds-upload rerun.
