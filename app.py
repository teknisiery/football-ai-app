...
# -----------------------------------------------------------
# Task 8: Load league round config dari file eksternal
# -----------------------------------------------------------
...
# Fungsi parsing odds correct score
def parse_odds_csv(file_content: bytes) -> dict:
    """Mengubah file CSV odds menjadi dictionary {skor: odds}."""
    df = pd.read_csv(BytesIO(file_content))
    odds_dict = {}
    # Cari kolom yang mengandung 'Score' dan 'Odds' (case-insensitive)
    score_col = None
    odds_col = None
    ...
    if score_col is None or odds_col is None:
        # Fallback: gunakan kolom kedua dan ketiga jika ada tiga kolom
        if len(df.columns) >= 3:
...
def get_cs_recommendations(score_probs: List[Tuple[int, int, float]], odds_dict: dict):
    """Kembalikan maksimal 3 skor dengan nilai expected value tertinggi."""
    candidates = []
    for h, a, prob in score_probs:
        key = f"{h}:{a}"
        if key in odds_dict:
            ev = prob * odds_dict[key] - 1
            candidates.append((h, a, odds_dict[key], ev, prob))
...
class PredictionResult:
    ...
    score_probs: Optional[List[Tuple[int, int, float]]] = None  # seluruh probabilitas skor
...
class Predictor:
    ...
    def predict(self, features_df: pd.DataFrame) -> PredictionResult:
        ...
        # Simpan seluruh probabilitas skor untuk correct score
        ...
        # Task 10: Boboti skor dengan probabilitas BTTS
...
class AppState:
    ...
    uploaded_odds: Optional[dict] = None  # dictionary odds correct score
...
def render_prediction_card(summary: dict):
    ...
    if summary.get('cs_recommendations'):
        ...
        for i, (h, a, odds, _, _) in enumerate(recs):
            stake = stakes[i]
...
def serialize_prediction(result: PredictionResult) -> dict:
    return {
        ...
        'score_probs': result.score_probs  # simpan seluruh probabilitas skor
    }
...
def get_match_prediction_summary(session: SessionManager, storage: StorageProvider):
    ...
    # Rekomendasi Correct Score jika odds tersedia
    cs_recs = None
    odds_dict = session.uploaded_odds
    if odds_dict and r.get('score_probs'):
        cs_recs = get_cs_recommendations(r['score_probs'], odds_dict)
...
def render_cs_history(session: SessionManager, storage: StorageProvider):
    ...
    # Hitung win rate (jika salah satu prediksi tepat)
    wins = 0
    total_bets = len(df)
    for _, row in df.iterrows():
        home_goals = int(row.get('home_goals', 0) or 0)
...
def main():
    ...
    if session.uploaded_file is None:
        ...
        if session.get_prediction_state().processed:
            # Upload odds correct score setelah prediksi
            st.markdown("---")
            ...
            if odds_file:
                ...
                # Jika belum upload, kosongkan odds
                session.uploaded_odds = None
