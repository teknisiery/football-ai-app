...
def main():
    ...
    with st.sidebar:
        ...
        # ---- Edit Statistik Liga (langsung reaktif, tanpa tombol Load) ----
        with st.expander("✏️ Edit Statistik Liga"):
            ...
            if profil_league.empty:
                ...
                # Gunakan callback untuk menyimpan kode liga yang dipilih
                def on_league_change():
                    selected_str = st.session_state["edit_league_select"]
                    if selected_str:
                        st.session_state["edit_league_code"] = int(selected_str.split(']')[0].replace('[', ''))

                st.selectbox("Pilih Liga", league_options, key="edit_league_select", on_change=on_league_change)

                selected_code = st.session_state.get("edit_league_code", None)
                if selected_code is not None:
                    ...
                    if mask.any():
                        ...
                        st.error("Kode liga tidak ditemukan.")
