        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({row['prediction_status']})"):
            ...
            if st.button("💾 Save Score", key=f"save{idx}"):
                ...
                if valid:
                    ...
                    # Gunakan record lengkap dari pending (sudah berisi semua fitur)
        ...
        if session.get_prediction_state().processed:
            ...
            if summary:
                ...
                if st.button("💾 Save Pending"):
                    ps = session.get_prediction_state()
                    # Data fitur lengkap dari prediction_dataframe (setelah add_features + attach profile)
                    ...
                    # Pastikan kolom penting ada (league_name, dll)
