...
PROFIT_SUMMARY_FILE = BASE_DIR / "profit_summary.json"  # Cache ringkasan liga
...
def get_valid_time(row, primary_col='kickoff_time', secondary_col='settlement_time'):
    """Ambil timestamp valid pertama dari dua kolom."""
...
def compute_profits_by_league(history_df: pd.DataFrame):
    ...
    # Task 11.A: normalisasi nama liga
    ...
    # Simpan ringkasan ke file cache
...
# -----------------------------------------------------------
# Task 11.B: Fitur tambah liga baru
# -----------------------------------------------------------
def add_new_league(league_code, league_name, avg_goals, over25_pct, btts_pct, under35_pct,
    ...
    # 1. Update profil_league.csv di database
    try:
    ...
    # 2. Update league_round_config.json lokal
    config_file = BASE_DIR / "league_round_config.json"
    if config_file.exists():
        with open(config_file) as f:
            config = json.load(f)
    ...
    # 3. Perbarui variabel global LEAGUE_ROUND_CONFIG
...
# ============================================================
# PREDICTION ENGINE (dengan Task 10)
# ============================================================
@dataclass
...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({info})"):
            ...
            # --- Tombol Save dengan callback (Task C) ---
            ...
            # --- Live Bet dibungkus expander (Task B) ---
...
def main():
    ...
    with st.sidebar:
        ...
        # --- Performa per Liga (dengan collapse & cache) ---
            ...
            with st.expander("📊 Performa per Liga (klik untuk lihat)"):
                for league in sorted(league_data_display.keys()):
                    ...
                    with st.expander(f"{league} - Rp {data['profit']:+,.0f}"):
                        ...
                        # Profit per status tidak tersedia di ringkasan, jadi kita lewati
