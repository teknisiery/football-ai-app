...
def parse_odds_csv(file_content: bytes) -> dict:
    ...
    for _, row in df.iterrows():
        try:
            ...
            type_str = str(row.iloc[0]).strip().lower()  # kolom pertama adalah Type

            if 'home' in type_str:
                # Home menang, skor sudah perspektif home
                odds_dict[score_str] = odds_val
            elif 'away' in type_str:
                # Away menang, skor adalah dari sisi away, perlu dibalik
                a_goals, h_goals = map(int, score_str.split(':'))
                odds_dict[f"{h_goals}:{a_goals}"] = odds_val
            elif 'draw' in type_str:
                # Draw, skor sudah simetris
                odds_dict[score_str] = odds_val
            # abaikan 'Other' atau type lain
        except:
            pass
    return odds_dict
...
def render_cs_history(session: SessionManager, storage: StorageProvider):
    ...
    # Tentukan kolom yang akan ditampilkan dengan aman
