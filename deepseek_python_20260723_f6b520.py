...
# + Sidebar: Total Profit All Time & per Bulan
# ============================================================
...
def compute_profits(history_df: pd.DataFrame) -> Tuple[float, pd.DataFrame]:
    """Hitung total profit all-time dan profit per bulan."""
    if history_df.empty:
        return 0.0, pd.DataFrame(columns=["Bulan", "Profit"])
    ...
    for _, row in history_df.iterrows():
        ...
        if actual is None:
            ...
            pred = 1 if pred_str.startswith('OVER') else 0
            win = (pred == actual)
            odds = row.get('current_over_odds') if pred == 1 else row.get('current_under_odds')
            stake = 100000
            profit = stake * (odds - 1) if win else -stake
        profits.append(profit)
    
    history_df = history_df.copy()
    history_df['profit'] = profits
    total_profit = sum(profits)
    
    # Agregasi per bulan
    if 'settlement_time' in history_df.columns:
        history_df['settlement_time'] = pd.to_datetime(history_df['settlement_time'], errors='coerce')
        history_df['bulan'] = history_df['settlement_time'].dt.strftime('%Y-%m')
        monthly = history_df.groupby('bulan')['profit'].sum().reset_index()
        monthly.columns = ['Bulan', 'Profit']
        monthly = monthly.sort_values('Bulan')
    else:
        monthly = pd.DataFrame(columns=["Bulan", "Profit"])
    
    return total_profit, monthly
...
def run_analysis(session: SessionManager, storage: StorageProvider, predictor: Predictor):
    ...
    # Tambahkan home_team dan away_team
    if 'home_team' not in df.columns:
        df['home_team'] = home
    if 'away_team' not in df.columns:
        df['away_team'] = away
    
    # Hapus league_name dan reorder sebelum menyimpan dataset
    df_clean = df.copy()
        ...
        with st.expander(f"▶ {home_safe} vs {away_safe} ({row['prediction_status']})"):
            ...
            if st.button("💾 Save Score", key=f"save{idx}"):
                ...
                if valid:
                    ...
                    # Buat DataFrame dari full_record dan bersihkan
...
def main():
    ...
    with st.sidebar:
        ...
        total_profit, monthly_profit = compute_profits(hist_df)
        st.metric("Total Profit (All Time)", f"Rp {total_profit:+,.0f}")
        if not monthly_profit.empty:
            st.dataframe(monthly_profit, height=200, use_container_width=True)
        else:
