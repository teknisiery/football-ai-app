...
# + Tampilan skor kosong (None) & kunci berbasis match_uid
# ============================================================
...
def render_pending(session: SessionManager, storage: StorageProvider):
    ...
    for idx, row in df.iterrows():
        ...
        match_uid = row['match_uid']  # gunakan sebagai kunci unik
        with st.expander(f"▶ {home_safe} vs {away_safe} ({row['prediction_status']})"):
            # Siapkan nilai awal, None jika data kosong
            ...
            save_key = f"save_disabled_{match_uid}"
            if save_key not in st.session_state:
                st.session_state[save_key] = False

            if st.button("💾 Save Score", key=f"save_{match_uid}", disabled=st.session_state[save_key]):
